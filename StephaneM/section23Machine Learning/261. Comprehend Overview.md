## 1. Amazon Comprehend가 뭐야?

* **자연어 처리(NLP) 서비스**
* 텍스트를 **분석·이해**해서 구조화된 정보로 변환
* 완전 관리형, 서버리스 → 별도 서버 구성 필요 없음

---

## 2. Comprehend가 할 수 있는 것

### ① 언어 감지 (Language Detection)

* 입력 텍스트가 어떤 언어인지 자동 감지

### ② 키워드·엔티티 추출 (Key Phrases & Entities)

* 사람, 장소, 브랜드, 사건 등 텍스트 속 중요한 정보 추출

### ③ 감정 분석 (Sentiment Analysis)

* 텍스트가 **긍정, 부정, 중립, 혼합** 중 어떤 감정을 담고 있는지 판단

### ④ 토픽 모델링 (Topic Modeling)

* 문서 여러 개를 자동으로 주제별로 분류
* 기사, 리뷰, 이메일 등 대량 텍스트 정리 가능

### ⑤ 문장 구조 분석

* 토큰화(Tokenization)
* 품사(Parts of Speech) 분석
* 오디오 분석 지원
...
* 고객 이메일, 리뷰 분석 → 긍정/부정 요인 파악
* 비즈니스 인사이트 도출 → 서비스 개선

### 🔹 문서 자동 분류

* 뉴스 기사, 블로그, FAQ 등 다량 문서를 주제별 자동 그룹화
...
| ---------- | ----------------- |
| **언어 감지**  | 텍스트 언어 자동 인식      |
| **엔티티 추출** | 사람, 장소, 브랜드, 사건 등 |
| **감정 분석**  | 긍정/부정/중립/혼합 판단    |
| **토픽 모델링** | 문서 자동 주제 분류       |
| **NLP**    | 자연어 처리 핵심 서비스     |

* Amazon Comprehend = **시험에서 NLP**
* 완전 관리형 + 서버리스
* 대량 텍스트 분석 → 의미/감정/주제 추출


```md
# Comprehend Overview
# Comprehend 개요
# → Amazon Comprehend 서비스 소개

Now, Amazon Comprehend is a very simple service from an exam perspective.  
지금, Amazon Comprehend는 시험 관점에서 매우 간단한 서비스입니다.  
→ 시험용 핵심 포인트 강조
...
요약하면, 시험 관점에서 Amazon Comprehend는 NLP 관련하여 반드시 기억해야 할 서비스입니다.  
→ 시험용 핵심 정리
...
Amazon Comprehend는 완전 관리형 서버리스 자연어 처리(NLP) 서비스입니다.  
→ 서버리스 + NLP 강조
...
머신러닝을 사용하여 텍스트에서 언어 감지, 주요 구문, 엔티티, 감정, 주제 등의 인사이트를 추출합니다.  
→ 주요 기능 요약

Comprehend can analyze text to understand sentiment and organize large collections of documents by topics.  
Comprehend는 텍스트 분석을 통해 감정을 이해하고, 많은 문서를 주제별로 정리할 수 있습니다.  
→ 분석 + 주제 분류
...
일반적인 사용 사례로는 고객 상호작용 분석을 통한 비즈니스 인사이트 획득과 발견된 주제별 기사 그룹화가 있습니다.  
→ 실무 활용 예시
```

🎮 **게임보상: Comprehend NLP 마스터 🏆**
→ NLP 기반 텍스트 분석, 감정 분석, 토픽 모델링 이해 완료
