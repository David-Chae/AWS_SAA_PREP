## 1. Amazon Transcribe가 뭐야?

* **음성 파일이나 실시간 음성 스트림을 텍스트로 변환**
* 마이크, 통화 녹음, 영상 오디오 모두 가능
...
### 핵심 기술: ASR (Automatic Speech Recognition)

* 딥러닝 기반 음성 인식 기술
* 억양, 발음 차이도 최대한 보정해서 인식

👉 사람이 받아 적는 걸 AI가 대신함

---

## 3. Amazon Transcribe의 중요한 기능들

### ① PII 자동 제거 (PII Redaction) ⭐ 시험 중요
...
### ② 자동 언어 인식 (Automatic Language Identification)

* 음성에 여러 언어가 섞여 있어도 자동 감지
* 영어 → 프랑스어 → 스페인어 전환해도 인식
...
* 영상 속 말까지 텍스트로 만들어서
* “그 사람이 이 말을 한 장면” 검색 가능

👉 **아카이브 검색 혁신**

---

## 5. 실제 사용 흐름 (그림으로 그리면 이거)
...
Amazon Transcribe는 Automatic Speech Recognition(ASR)이라는 딥러닝 프로세스를 사용하여 음성을 빠르고 정확하게 텍스트로 변환합니다.  
→ 딥러닝 기반 ASR

There are several important features to be aware of.  
알아두어야 할 중요한 기능이 몇 가지 있습니다.  
→ 기능 강조
...
핵심 기능 중 하나는 개인 식별 정보(PII)를 자동으로 제거하는 것입니다.  
→ 민감 정보 보호
...
즉, 오디오에 나이, 이름, 주민등록번호 등 민감한 데이터가 포함되어 있으면 Amazon Transcribe가 자동으로 이를 제거할 수 있습니다.  
→ 자동 PII 필터링
...
Amazon Transcribe는 다국어 오디오에서 언어를 자동으로 인식할 수 있습니다.  
→ 다국어 자동 감지
...
예를 들어, 오디오에 프랑스어, 영어, 스페인어가 포함되어 있으면 서비스가 이를 모두 인식합니다.  
→ 실시간 언어 감지

## Use Cases for Amazon Transcribe
## Amazon Transcribe 활용 사례
# → 실무 적용
...
PII와 같은 특정 내용을 제거하는 설정을 활성화할 수 있습니다.  
→ 설정 가능
...
예를 들어, "Hello, my name is Stephane, I am 31 years old, and my phone number is 910-747-280"라고 하면 서비스가 이름과 전화번호를 자동으로 숨깁니다.  
→ 자동 PII 제거 예시

Note that the phone number used in this example is not real and cannot be used.  
이 예제의 전화번호는 실제 번호가 아니며 사용 불가합니다.  
→ 예시 안내
...
Amazon Transcribe는 여러 언어의 스트리밍을 동시에 지원합니다.  
→ 동시에 여러 언어 처리
...
다국어 오디오에 대한 자동 언어 인식을 지원하며, 영어, 프랑스어, 스페인어를 인식합니다.  
→ 다국어 자동 감지
...
일반적인 활용 사례로 고객 통화 기록 전사, 자동 자막 생성, 미디어 자산 검색 가능한 메타데이터 생성 등이 있습니다.  
→ 활용 사례
```

🎮 **게임보상: Transcribe 마스터 🏆**
→ 음성 전사, PII 제거, 다국어 전사까지 완벽 습득
