...
## 1️⃣ CloudWatch Agent란?

**한 줄 요약:**
👉 EC2 인스턴스나 온프레미스 서버에서 **로그와 시스템 지표(metrics)를 CloudWatch로 보내주는 프로그램**

* 기본적으로 EC2는 로그나 상세 메트릭을 CloudWatch에 보내지 않음
* Agent를 설치해야 로그와 상세 메트릭 수집 가능
* Agent 설치를 위해 EC2에는 **CloudWatch에 데이터를 보낼 권한(IAM 역할)** 필요
...
| Agent                        | 특징                                                             |
| ---------------------------- | -------------------------------------------------------------- |
| **CloudWatch Logs Agent**    | 오래된 버전, **로그만** 전송 가능                                          |
| **CloudWatch Unified Agent** | 최신 버전, **로그 + 상세 시스템 지표** 수집 가능, 중앙 구성 가능(SSM Parameter Store) |

* **Unified Agent**가 이름 그대로 “통합” 기능 제공
* 중앙에서 구성 가능 → 여러 서버에 동일 설정 배포 용이

---

## 3️⃣ Unified Agent가 수집 가능한 메트릭

### CPU 관련

* 상태별 CPU 사용량: `active`, `guest`, `idle`, `system`, `user`, `steal`
...
* TCP/UDP 연결 수, 패킷 수, 바이트 수

### 프로세스 관련

* 전체 프로세스 수 및 상태: `running`, `sleeping`, `idle`, `blocked`, `dead`

---

## 4️⃣ 왜 Unified Agent를 쓰는가?

* EC2 기본 모니터링: CPU, 디스크, 네트워크 정도만 제공, 메모리/Swap는 없음
* Unified Agent 설치 시: 훨씬 **상세하고 세분화된 지표** 수집 가능
* 실무에서 성능 분석, 문제 진단, 용량 계획에 유용

---

## 5️⃣ 온프레미스 서버에서도 가능

* VMware, 물리 서버 등에도 동일 Agent 설치 가능
* 로그와 메트릭을 CloudWatch에 전송 가능
...
| 항목            | 내용                                       |
| ------------- | ---------------------------------------- |
| 용도            | EC2/온프레미스 서버 로그 및 메트릭 수집                 |
| Logs Agent    | 로그만 전송                                   |
| Unified Agent | 로그 + 상세 지표 수집, 중앙 구성 가능                  |
| 수집 지표         | CPU, Disk, RAM, Swap, Network, Process 등 |
| 장점            | EC2 기본 모니터링보다 훨씬 상세, 실시간 문제 감지 가능        |

---

💡 **Tip:**
시험에서는 핵심 포인트만 기억하면 됩니다:

> **“Logs Agent = 로그만, Unified Agent = 로그 + 상세 메트릭 + 중앙 구성 가능”**
