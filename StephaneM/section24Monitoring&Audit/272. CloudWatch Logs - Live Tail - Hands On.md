...
**한 줄로 요약**
👉 **로그가 찍히는 순간, 실시간으로 바로 보는 기능**

* 말 그대로 `tail -f` 같은 느낌
* 디버깅할 때 특히 유용
* 로그가 들어오는 속도를 지켜볼 수 있음

---

## 2️⃣ 기본 사용 흐름

### ① Log Group & Log Stream 생성

* **Log Group**: 로그를 모아두는 폴더

  * 예: `demo-log-group`
* **Log Stream**: 실제 로그가 쌓이는 파일

  * 예: `DemoLogStream`

> Live Tail은 이 Log Stream 단위로 실시간 스트리밍 가능

---

### ② Live Tail 시작

1. Log Stream 안에서 **Start Tailing** 클릭
2. Live Tail UI가 열림 → 로그 이벤트 대기
3. 필요하면 **로그 그룹 필터** 및 **로그 스트림 필터** 적용

   * 스트림 지정 안 해도 로그 그룹 전체를 볼 수 있음

---

### ③ 로그 이벤트 실시간 확인

* 로그를 생성하거나 들어오는 순간, Live Tail에서 바로 표시
* 예시:

  1. `Create Log Event` 선택
  2. 메시지 입력: `"hello world"`
  3. Live Tail 화면에 즉시 `"hello world"` 표시

📌 특징

* **타임스탬프, 로그 그룹 정보** 같이 상세 표시
* 클릭하면 해당 로그 스트림으로 바로 이동 가능
...
2. **상세 정보 제공**
   ...
   * 타임스탬프, 로그 그룹, 스트림 링크 등

3. **편리함**

   * 별도의 로그 다운로드 없이 바로 확인 가능
...
* **무료 사용량**: 하루 약 **1시간**
* 사용 후 **세션 종료 필수** → 과금 방지
* 실시간 모니터링 + 무료 1시간 → 작은 환경에서는 거의 비용 걱정 없음
...
| ----- | ----------------------------- |
| 목적    | 로그 실시간 스트리밍 & 디버깅             |
| 필요 조건 | Log Group + Log Stream        |
| 필터    | 로그 그룹 필터, 선택적 로그 스트림 필터       |
| 장점    | 즉시 확인, 타임스탬프/상세 정보, 스트림 링크 제공 |
| 비용    | 하루 1시간 무료, 사용 후 종료            |

---

💡 **Tip:**
실무에서는 배포 후 오류 발생 시, Live Tail로 바로 로그 확인 → 원인 분석 → 빠른 대응 가능.
시험에서는 **“실시간 로그 확인, 필터 적용 가능, 하루 1시간 무료”** 정도만 기억하면 충분.
