## 1️⃣ CloudWatch Alarm이란?

**한 줄 요약:**
👉 특정 메트릭이 지정한 기준(threshold)을 초과하거나 조건을 만족할 때 **자동으로 알림이나 작업을 수행하는 기능**

예: CPU 사용률이 80%를 초과하면 경고 메일을 받거나, EC2를 자동으로 재시작.

---

## 2️⃣ Alarm 상태(State)

CloudWatch Alarm은 총 3가지 상태를 가집니다:

| 상태                    | 의미                          |
| --------------------- | --------------------------- |
| **OK**                | 메트릭이 기준 내에 있어 알람이 트리거되지 않음  |
| **INSUFFICIENT_DATA** | 데이터가 부족하여 상태 판단 불가          |
| **ALARM**             | 기준을 초과하여 알람이 트리거됨, 알림 전송 가능 |

---

## 3️⃣ Alarm 평가 기간(Evaluation Period)

* Alarm이 메트릭을 확인하는 기간
* 예: 10초, 30초, 60초 등
* 고해상도(custom high-resolution) 메트릭도 지원

> 짧게 설정하면 빠르게 반응, 길게 설정하면 일시적인 변동으로 인한 알람 방지

---

## 4️⃣ Alarm 동작(Targets)

CloudWatch Alarm은 기준 충족 시 다음과 같은 동작을 수행할 수 있습니다:

1. **EC2 Instance Actions**

   * 인스턴스 **중지, 종료, 재부팅, 복구** 가능

2. **Auto Scaling Actions**

   * Auto Scaling 그룹을 **확장/축소**

3. **Notifications**

   * Amazon SNS로 알림 전송 → 필요 시 Lambda 트리거 가능

---

## 5️⃣ Composite Alarm

* 여러 개의 알람 상태를 **논리 연산(AND/OR)**으로 결합
* 장점: 불필요한 알람 줄이기
* 예:

  * Alarm A: CPU > 80%
  * Alarm B: IOPS > 1000
  * Composite Alarm: **A AND B** → 둘 다 조건 충족 시만 알람 발생

---

## 6️⃣ EC2 인스턴스 복구

CloudWatch Alarm을 사용하여 **EC2 자동 복구** 가능:

* 상태 체크 종류:

  1. **Instance Status Check**: VM 자체 확인
  2. **System Status Check**: 하드웨어 확인
  3. **EBS Status Check**: 연결된 EBS 확인

* 알람 조건 충족 시, **다른 호스트로 인스턴스 이동** 가능

* IP, 메타데이터, Placement Group 등 그대로 유지

---

## 7️⃣ CloudWatch Logs 기반 Alarm

* **Metric Filters**와 결합 가능

* 예: 로그에 `"error"`가 많이 발생하면 알람 트리거 → SNS 알림

* 테스트 방법: CLI로 **set-alarm-state** 사용 → 임의로 알람 상태 변경 가능
...
| 항목              | 내용                                      |
| --------------- | --------------------------------------- |
| 용도              | 메트릭을 기준으로 자동 알람 및 작업 수행                 |
| 상태              | OK / INSUFFICIENT_DATA / ALARM          |
| 동작              | EC2 재부팅/종료/복구, Auto Scaling, SNS 알림 등   |
| Composite Alarm | 여러 알람 결합, 논리 연산으로 알람 발생 조건 세분화          |
| Logs 기반         | Metric Filter + Alarm → 로그 이벤트 기반 알람 가능 |
...
> **“CloudWatch Alarm = 메트릭 기준 알람 / 상태 3개 / EC2, Auto Scaling, SNS 동작 / Composite Alarm = 다중 조건 결합”**
