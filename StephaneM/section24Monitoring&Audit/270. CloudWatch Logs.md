## 1. CloudWatch Logs 개념

* **Log Groups**: 애플리케이션 단위로 로그를 그룹화
* **Log Streams**: 그룹 내 개별 로그 흐름 (예: 특정 파일, 컨테이너)
* **Retention**: 로그 보존 기간 설정 가능 (1일 ~ 10년 또는 무제한)
* **암호화**: 기본 제공 암호화 + KMS 커스텀 키 가능
...
* **AWS 서비스 직접 전송**
  ...
  * Elastic Beanstalk → 애플리케이션 로그
  * ECS → 컨테이너 로그
  * Lambda → 함수 로그
  * VPC Flow Logs → 네트워크 트래픽 메타데이터
  * API Gateway → 요청 로그
  * CloudTrail → 이벤트 로그
  * Route53 → DNS 쿼리 로그
* **수집 방식**
...
* 기능:
  ...
  * 이벤트 집계, 정렬, 필터링 가능
  * 시각화 → 대시보드 추가 가능
  * 다수 로그 그룹 또는 계정/리전 간 쿼리 가능
* 주의: **실시간 처리 아님** → 쿼리 시점의 데이터 분석

---

## 4. 로그 내보내기 및 실시간 스트리밍

### 배치(export)

* Amazon S3로 배치 내보내기 → 최대 12시간 소요
* API: `CreateExportTask` 사용

### 실시간/준실시간 스트리밍

* **Subscription Filters** 설정 → 실시간 전달
* 목적지:

  * Kinesis Data Streams → Firehose, Analytics, EC2, Lambda 연계
  * Kinesis Data Firehose → S3, OpenSearch, Lambda
  * Lambda → 맞춤 처리 후 OpenSearch 등으로 전달 가능
* 다계정 로그 집계 가능 → 중앙집중식 분석

### Cross-Account 스트리밍 설정

1. 송신 계정 → 구독 필터 생성, 목적지(destination) 지정
2. 수신 계정 → IAM 역할 생성, 송신 계정에 권한 부여
3. 송신 계정 로그 → 수신 계정 Kinesis Data Stream으로 스트리밍 가능
...
| 개념                   | 설명                                         |
| -------------------- | ------------------------------------------ |
| Log Groups           | 애플리케이션/서비스 단위 로그 그룹                        |
| Log Streams          | 그룹 내 개별 로그 흐름                              |
| Retention            | 로그 보존 기간 설정 (1일~10년/무제한)                   |
| Logs Insights        | 쿼리 및 시각화, 다계정·다로그 그룹 가능                    |
| Export               | 배치 → S3, 실시간 → Kinesis, Lambda, OpenSearch |
| Subscription Filters | 선택적 실시간 스트리밍, Cross-Account 가능             |

* CloudWatch Logs = **AWS 로그 저장, 분석, 스트리밍 핵심 서비스**
* 시험 TIP: “Log Groups/Streams, Retention, Insights, Export/Subscription” 기억
