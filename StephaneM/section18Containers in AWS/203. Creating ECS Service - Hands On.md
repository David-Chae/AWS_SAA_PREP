# 🚀 **ECS 서비스 만들기 – 완전 쉽게 이해하기**

ECS에서 컨테이너를 돌리려면 크게 두 가지 단계가 필요해:

1. **Task Definition 만들기**
2. **Service 만들기**

이 두 가지를 합쳐서 “컨테이너가 어떻게 실행될지”를 결정한다고 보면 돼.

---

# ✅ 1. **Task Definition(태스크 정의) 만들기**

**Task Definition은 “컨테이너 레시피” 같은 것**이야.
컨테이너를 어떻게 만들고 얼마나 자원을 쓸지 정리한 설명서라고 보면 된다.

이번 실습에서는 **nginxdemos-hello** 라는 이름으로 Task Definition을 만든다.

---

## 🔹 1) 어떤 컨테이너 이미지를 쓸지 선택

* 도커 허브에 있는 **nginxdemos/hello** 이미지를 사용
  → 바로 Nginx 기반의 간단한 웹 페이지를 보여주는 이미지

---

## 🔹 2) 이 컨테이너를 어디서 돌릴지 선택 (Fargate or EC2)

여기서 Fargate만 쓸 것임 → **서버리스**, 서버 관리 필요 없음.

---

## 🔹 3) 컨테이너에게 줄 CPU, 메모리 설정
...
Fargate가 자동으로 필요한 서버 자원을 제공해준다.

---

## 🔹 4) IAM Role (권한)

* 컨테이너가 AWS API를 쓸 필요 없으니 task role은 비움
* 실행할 때 필요한 execution role은 자동 생성됨 → 그대로 둠
...
* 이름: nginxdemos-hello
* 이미지 URL: `nginxdemos/hello`
* Port: 80 → 80 (기본값 OK)
* 스토리지: 기본 21GB

이걸 저장하면 **Task Definition이 완성**된다.
...
> “이 태스크를 몇 개, 어디에, 어떻게 계속 실행할지 관리하는 관리자”

Task Definition이 ‘레시피’라면
Service는 ‘레시피로 요리를 몇 개 만들지, 계속 유지시킬지 관리하는 시스템’이야.
...
## 🔹 2) 어떤 Task Definition을 사용할지 선택

* 아까 만든 **nginxdemos-hello**
* 최신 revision 선택

서비스 이름은 기본값 그대로 해도 됨.
...
* Fargate 선택
* Capacity provider 전략을 기본값으로 둔다

---

## 🔹 4) 몇 개의 컨테이너(task)를 돌릴지

* 처음에는 1개만 실행
  (나중에 쉽게 3개, 4개 등으로 늘릴 수 있음)
...
웹서비스니까 **로드 밸런서가 있어야 해**
사용자 요청을 여러 컨테이너에 분배해 주거든.

설정한 내용:

* ALB 생성 → 이름: DemoALBForECS
* Listener: 80
* Target Group 생성 → 이름: nginxdemosTG

로드 밸런서가 Fargate 컨테이너의 IP를 자동으로 타깃으로 등록함.

이제 서비스 생성 버튼 클릭!
→ ECS가 자동으로 컨테이너 1개를 Fargate에서 실행시킴.

---

# 🎉 **배포 완료 → 브라우저로 확인**

로드 밸런서 DNS 주소를 브라우저에서 열면

> Nginx 데모 페이지가 뜬다!

즉, Fargate에서 돌아가는 컨테이너가 성공적으로 서비스되고 있다는 뜻.
...
서비스의 강력한 기능 중 하나가 바로 스케일링(확장)이야.

### ▪️ 늘리기 (예: 1 → 3)

* 서비스 설정에서 desired count를 3으로 변경
* Fargate가 자동으로 컨테이너 2개 추가 실행
* ALB는 3개 컨테이너로 트래픽을 분산

### ▪️ 줄이기 (예: 3 → 0)

* desired count를 0으로 설정하면 모든 컨테이너 종료
* 하지만 서비스 자체는 남아있음

필요 없는 EC2 인스턴스가 있다면 ASG도 0으로 바꾸면 비용 절약됨.

---

# 📌 **핵심 요약**

* **Task Definition** → 컨테이너가 어떤 이미지로 어떻게 실행될지 정의 (레시피)
* **Service** → Task를 몇 개 실행하고 유지할지 관리하는 시스템
* Fargate 사용 → 서버리스, 설정 간단
* ALB 설정 → 외부에서 접속 가능
* 스케일링 매우 쉬움 → desired tasks 숫자만 바꾸면 됨
