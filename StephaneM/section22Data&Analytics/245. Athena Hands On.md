# 📘 Athena Hands-On 쉽게 이해하기

## 1) Athena 쿼리 준비

1. **Athena 쿼리 에디터 실행** → SQL 쿼리를 작성할 수 있는 UI
2. **쿼리 결과 위치 지정**

   * Athena가 쿼리 결과를 저장할 **S3 버킷 필요**
   * 예시: `aws-athena-stephane` (EU Central 1)
   * 버킷 이름은 전 세계에서 유일해야 함
   * 설정 완료 후, 모든 쿼리 결과가 이 버킷에 저장됨

> Athena는 서버리스라서 직접 DB를 만들 필요 없이 S3에 저장된 데이터만 있으면 바로 쿼리 가능
...
1. **데이터베이스 생성**
   ...
   * S3 데이터 조회용으로 DB를 만든다고 생각하면 됨

2. **테이블 생성**

   * S3 버킷 안에 있는 **액세스 로그** 데이터를 기준으로 테이블 정의
   * SQL 예시는 AWS 문서에서 제공
   * 버킷 경로 지정만 하면 됨

3. **테이블 확인 및 미리보기**

   * 쿼리로 첫 10개 행 확인 → 버킷 이름, 요청 시간, IP, 요청자, 요청 ID 등 표시

> 테이블은 Athena 내부에 논리적으로 존재, 실제 데이터는 S3에 있음

---

## 3) 데이터 분석

* Athena를 통해 **SQL 쿼리로 직접 분석 가능**
* 예시: HTTP 요청 상태별 요청 수 확인
  ...
  ```
* **보안 분석:** HTTP 403 필터링 → 무단 접근 시도 감지

> 직접 파일을 열어보지 않아도 S3 전체 로그를 빠르게 분석 가능
...
* 결과 자동 S3 저장 → 다른 분석이나 BI 도구와 연동 용이

---

## 5) 실습 핵심 요약

1. **S3 버킷 생성** → 쿼리 결과 위치로 설정
2. **Athena DB 생성** → S3 데이터를 조직화
3. **테이블 생성** → S3 객체 구조에 맞춰 정의
4. **쿼리 실행** → 데이터 조회 및 분석
5. **고급 분석** → 상태 코드별 요청 수, 보안 이상 탐지 등

---

# 📌 한 줄 요약

**Athena Hands-On = S3 데이터에 SQL로 접근하고, 서버리스 환경에서 직접 쿼리·분석하는 과정**
