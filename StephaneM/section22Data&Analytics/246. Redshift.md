...
# 📘 Amazon Redshift 쉽게 이해하기

## 1) Redshift란?

* **목적:** OLAP(온라인 분석 처리)용 데이터베이스
  → SQL 기반으로 대규모 데이터 분석과 보고에 적합
* **PostgreSQL 기반**이지만, 일반 트랜잭션 처리(OLTP)용은 아님
* **특징:**

  * 컬럼형 저장(Columnar Storage) → 특정 컬럼만 읽어 빠른 집계 가능
  * 병렬 쿼리 처리(Parallel Query Engine) → 대규모 데이터도 빠르게 분석
  * 대규모 데이터 지원 → 페타바이트(PB) 단위까지 가능

---

## 2) Redshift와 Athena 비교

| 특징     | Redshift       | Athena                  |
| ------ | -------------- | ----------------------- |
| 서버 관리  | 클러스터 필요        | 서버리스                    |
| 쿼리 속도  | 빠름, 인덱스와 병렬 처리 | S3 데이터를 직접 스캔, 느림       |
| 데이터 로딩 | 데이터 적재 필요      | 데이터 이동 없이 바로 쿼리         |
| 활용     | 복잡한 분석, 조인, 집계 | 가벼운 ad hoc 분석, S3 로그 분석 |
...
* **컴퓨트 노드(Compute Nodes)**: 실제 쿼리 수행 → 리더 노드로 결과 전송

* **쿼리 처리 예시:**

  ```sql
  SELECT COUNT(*) FROM My_TABLE GROUP BY na;
  ```

  → 리더가 계획 → 컴퓨트 노드 실행 → 리더가 최종 결과 반환

* **운영 모드:**

  * **Provisioned:** 사용자가 인스턴스 타입 선택, 예약 가능
  * **Serverless:** AWS가 클러스터 관리, 자동 확장

---

## 4) 백업과 재해복구

* **스냅샷(Snapshots)**: 특정 시점의 백업, S3에 저장, 증분 저장 방식

  * **자동 스냅샷:** 주기적 실행, 보존 기간 설정 가능
  * **수동 스냅샷:** 수동 삭제 전까지 보존
  * **크로스 리전 복사:** 다른 AWS 리전으로 복제 가능 → 재해복구

* **멀티 AZ:** 일부 클러스터에서 지원 → 기본 AZ 장애 대비

---

## 5) 데이터 적재 방법

1. **Amazon Kinesis Data Firehose:** 실시간 스트리밍 → S3 → Redshift
2. **수동 Copy 명령어:** S3 → Redshift 수동 적재
3. **JDBC 드라이버:** 애플리케이션에서 직접 Redshift에 배치 적재

> **보안 팁:** 기본적으로 인터넷을 거치지만, VPC 라우팅 활성화 시 데이터가 VPC 내부로만 흐름

---

## 6) Redshift Spectrum

* **S3 데이터 직접 조회:** Redshift 클러스터에 적재하지 않고도 S3 데이터를 분석 가능
* **작동 방식:**

  * 수천 개의 Spectrum 노드가 S3 데이터를 읽고 집계 → Redshift 클러스터로 전달
* **장점:** 대규모 S3 데이터도 효율적 분석 가능, 클러스터 용량 제한 없이 처리

---

## 7) 요약 및 핵심 포인트

* **Redshift = OLAP용 고성능 데이터 웨어하우스**
* 컬럼형 저장 + 병렬 쿼리 → 빠른 분석
* 리더 + 컴퓨트 노드 구조, 서버리스/프로비저닝 모드 지원
* 스냅샷 기반 백업 + 크로스 리전 복사 → 재해복구 가능
* Kinesis Firehose, JDBC, S3 Copy를 통해 데이터 적재 가능
* Redshift Spectrum으로 S3 데이터 분석 확장 가능
