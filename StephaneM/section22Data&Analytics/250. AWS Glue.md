...
* **종류:** 완전 관리형 서버리스 ETL 서비스
* **역할:**

  * 데이터 준비 및 변환
  * 분석용 데이터 적재
* **예시:** S3나 RDS 데이터 → 변환(필터링, 컬럼 추가 등) → Redshift 적재

> 쉽게 말해 **“AWS 내 데이터를 분석용으로 준비해주는 자동화 도구”**
...
* CSV → **Parquet** 변환
  ...
  * Parquet: 컬럼 기반 저장, Athena 같은 분석 서비스에서 쿼리 속도 향상
* Workflow: S3 CSV 파일 → Glue ETL → Parquet → S3 저장 → Athena로 빠르게 분석

---

## 3) 자동화 ETL

* **방법 1:** S3 이벤트 → Lambda → Glue ETL job
* **방법 2:** S3 이벤트 → EventBridge → Glue ETL job
* **결과:** 파일 업로드만 해도 자동으로 ETL 수행 가능

---

## 4) Glue Data Catalog

* **정의:** Glue가 데이터 소스를 스캔하고 **메타데이터**를 관리하는 카탈로그
* **지원 소스:** S3, RDS, DynamoDB, JDBC 기반 DB, 온프레미스 DB
* **내용:** 테이블, 컬럼, 데이터 타입 등 정보
* **활용:**

  * Glue ETL 작업
  * Athena, Redshift Spectrum, EMR 등 다른 AWS 분석 서비스와 통합

> Glue Data Catalog = **AWS 분석 서비스 공용 데이터 사전**
...
| 기능                     | 설명                                                         |
| ---------------------- | ---------------------------------------------------------- |
| **Job Bookmarks**      | 이전 처리 데이터 건너뛰기, 증분 처리 가능                                   |
| **Glue DataBrew**      | GUI 기반 데이터 정제/표준화 도구                                       |
| **Glue Studio**        | ETL 작업 GUI 생성, 실행, 모니터링                                    |
| **Glue Streaming ETL** | 실시간 ETL, Spark Structured Streaming 기반, Kinesis/MSK 데이터 처리 |

---

## 6) Glue 요약

* **Glue = 서버리스 ETL + 데이터 카탈로그 + 배치/스트리밍 지원**
* **주요 포인트:**

  * ETL 자동화 → S3/RDS → Redshift/S3
  * 데이터 형식 변환 → CSV → Parquet
  * Glue Data Catalog로 메타데이터 관리 → Athena, Spectrum, EMR 연동
  * 증분 처리, GUI 관리, 실시간 처리 지원

> 시험 포인트: **“서버리스 ETL, Parquet 변환, Glue Data Catalog, Streaming ETL”**
