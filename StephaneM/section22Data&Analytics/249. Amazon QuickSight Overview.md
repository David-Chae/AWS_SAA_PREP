# 📊 Amazon QuickSight 쉽게 이해하기

## 1) QuickSight란?

* **종류:** 서버리스, 머신 기반 BI 서비스
* **주요 기능:**

  * 데이터 시각화, 대시보드 작성
  * 자동 확장 가능, 웹 사이트 임베딩 가능
  * 세션 단위 과금 방식

> 쉽게 말해 **“AWS 데이터로 시각적 보고서를 손쉽게 만드는 서비스”**

---

## 2) 데이터 소스 연결

QuickSight는 다양한 데이터 소스를 지원합니다:

| 유형          | 예시                                                        |
| ----------- | --------------------------------------------------------- |
| AWS 데이터베이스  | RDS, Aurora, Redshift, Athena, S3, OpenSearch, Timestream |
| 외부 SaaS     | Salesforce, Jira                                          |
| 온프레미스/기타 DB | Teradata, JDBC 연결 가능                                      |
| 파일 기반       | Excel, CSV, JSON, TSV, EFS CLF 로그 등                       |

> **Tip:** 데이터 소스를 QuickSight에 직접 가져오면 **SPICE 엔진**으로 초고속 메모리 계산 가능

---

## 3) SPICE 엔진

* **정의:** QuickSight 내에서 데이터를 가져와 **메모리 기반으로 빠르게 계산**하는 엔진
* **제한:** 데이터베이스 연결만 할 경우는 SPICE 사용 불가
* **장점:** 수십억 개 행의 데이터도 빠르게 시각화 가능

---

## 4) 사용자, 그룹, 대시보드

* **사용자/그룹 관리:** QuickSight 자체 관리, IAM 사용자와 별개
* **대시보드:** 읽기 전용, 공유 가능
* **분석(Analysis):** 대시보드보다 더 상세, 필터, 파라미터, 정렬 포함
* 공유 방법: Analysis 또는 Dashboard를 특정 사용자/그룹과 공유 가능

> 예: 팀마다 권한을 달리해 일부 데이터만 보이도록 설정 가능

---

## 5) 통합 활용

* **QuickSight + Athena:** S3 데이터를 SQL로 쿼리 → 시각화
* **QuickSight + Redshift:** 데이터 웨어하우스 분석 후 대시보드 생성
* **QuickSight + Timestream:** 시계열 데이터 시각화

---

## 6) 요약

* **QuickSight = 서버리스 BI + 대시보드 + 시각화**
* **주요 특징:**

  * AWS 및 외부 데이터 소스 연결
  * SPICE 엔진으로 빠른 계산
  * 사용자/그룹별 공유 가능
  * 웹, SaaS, 온프레미스 데이터와 통합 가능

> 시험에서는 **“데이터 시각화, 대시보드 작성, SPICE 사용, Athena/Redshift 연동”**이 핵심 포인트
