...
* **정식명:** Elastic MapReduce
* **역할:** Hadoop, Spark, Presto, HBase, Flink 등 **빅데이터 처리 도구**를 AWS에서 쉽게 운영
* **주요 특징:**

  * 대용량 데이터 분석, 처리, 머신러닝, 웹 인덱싱 등 가능
  * 수백 대 EC2 인스턴스로 클러스터 구성 가능
  * 클러스터 자동 스케일링 지원, Spot 인스턴스 연동으로 비용 절감

> 시험에서는 “Hadoop, Spark 기반 빅데이터 클러스터” → EMR 연상

---

## 2) EMR 클러스터 구성

EMR 클러스터는 **세 종류의 노드**로 구성됩니다:

| 노드 종류       | 역할                | 특성                           |
| ----------- | ----------------- | ---------------------------- |
| Master Node | 클러스터 관리, 모든 노드 조율 | 장기 운영 필수                     |
| Core Node   | 작업 실행 + 데이터 저장    | 장기 운영 필수                     |
| Task Node   | 작업 실행 전용          | 선택 사항, Spot 인스턴스로 저렴하게 운영 가능 |

---

## 3) 인스턴스 구매 옵션

노드 유형별로 적합한 인스턴스 구매 방식:

| 구매 옵션     | 특징                     | 추천 노드            |
| --------- | ---------------------- | ---------------- |
| On-Demand | 안정적, 예측 가능, 절대 종료되지 않음 | 필수/중요 노드         |
| Reserved  | 1년 이상 사용, 비용 절감        | Master, Core 노드  |
| Spot      | 저렴하지만 중간에 종료될 수 있음     | Task 노드, 일시적 작업용 |

---

## 4) 클러스터 운영 방식

* **장기 클러스터(Long-running):** 장기간 사용, Reserved 인스턴스 활용
* **일시 클러스터(Transient):** 작업 후 해체, 비용 절감 목적

---

## 5) 요약

* **EMR = 빅데이터 클러스터 관리 서비스**
* **자동 설정 + 스케일링**으로 Hadoop/Spark 환경 구성 단순화
* **클러스터 구조:** Master, Core, Task 노드
* **노드 구매:** On-Demand, Reserved, Spot 활용 전략

> 한 줄 요약: **“AWS에서 복잡한 빅데이터 환경을 쉽게 만들고 운영할 수 있게 해주는 서비스”**
