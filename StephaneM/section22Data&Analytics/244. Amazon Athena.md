...
* **목적:** S3에 저장된 데이터를 다른 DB로 옮기지 않고 바로 분석
* **특징:** 서버리스 → 서버나 인프라 관리 필요 없음
* **언어:** SQL 사용 가능
* **엔진:** Presto 기반
* **지원 파일 형식:** CSV, JSON, ORC, Avro, Parquet

> 쉽게 말해, S3가 ‘데이터 창고’라면 Athena는 그 위에서 SQL을 바로 실행해주는 도구야.

---

## 2) 비용 구조

* 스캔한 데이터 용량만큼만 요금 발생 → 효율적으로 쿼리하고 데이터 저장하는 것이 중요

---

## 3) Athena의 대표 활용 사례

* **임시 조회 / 분석(Ad hoc query)**
* **BI 및 리포팅** → QuickSight와 연동
* **로그 분석** → VPC Flow Logs, ELB 로그, CloudTrail 등

> 시험 TIP: S3 데이터를 서버리스 SQL로 분석해야 하면 Athena를 떠올리자.
...
* CSV는 전체 스캔 → 불필요한 데이터까지 스캔됨
* Parquet, ORC 같은 컬럼형 파일 → 필요한 컬럼만 스캔 → 비용 절감 + 속도 향상

### ② 데이터 압축

* 압축하면 스캔 데이터가 줄어 비용과 시간 절감 가능

### ③ 데이터 파티셔닝

* S3 폴더 구조를 기준으로 파티셔닝 → 필요한 데이터만 스캔
* 예시: `/year=2023/month=12/day=12/` → 특정 날짜 조회 시 다른 폴더는 스캔 안 함

### ④ 파일 크기 최적화

* 작은 파일 많으면 성능 저하 → 128MB 이상 권장

---

## 5) Federated Query (연합 쿼리)

* S3 외부 데이터도 SQL로 조회 가능
* 연결 가능한 곳: DynamoDB, RDS, Redshift, Aurora, DocumentDB, ElastiCache, HBase, 심지어 온프레미스 DB까지
* Lambda 기반 커넥터를 통해 연결
* 결과는 다시 S3에 저장 가능

> 즉, Athena 하나로 AWS 안팎 데이터 통합 조회 가능

---

## 6) 핵심 요약

* **서버리스 SQL 서비스** → 인프라 관리 불필요
* **S3 데이터 바로 조회** → CSV, JSON, Parquet, ORC 등 지원
* **성능 최적화:** 컬럼형, 압축, 파티션, 큰 파일
* **Federated Query:** 다양한 AWS/온프레미스 데이터와 통합 쿼리 가능

---

# 📌 한 줄 요약

**Amazon Athena = S3에 있는 데이터를 서버리스 SQL로 바로 조회하고 분석할 수 있는 효율적 도구**
