...
* **AWS IoT Core**
  ...
  * 수많은 IoT 장치에서 데이터를 수집
  * 실시간으로 데이터를 AWS로 전송
* **Amazon Kinesis Data Streams**

  * IoT Core에서 받은 데이터를 고속으로 스트리밍
  * 대량의 데이터 처리 가능
...
* **Amazon Kinesis Data Firehose**
  ...
  * Data Streams에서 들어온 데이터를 S3 버킷으로 자동 배치 전송 (최소 1분 단위)
  * 필요 시 Lambda를 연결하여 **실시간 변환/클렌징**
* **AWS Lambda**

  * Firehose와 연결되어 데이터 변환
  * 또는 S3 버킷에 데이터가 들어올 때 트리거되어 Athena 쿼리 수행 가능
  * 선택적으로 SQS를 중간에 두어 메시징/처리 흐름 제어 가능
...
* **Amazon Athena**
  ...
  * S3에 저장된 데이터를 **서버리스 SQL**로 쿼리
  * 쿼리 결과를 **보고용 S3 버킷**에 저장
...
* **Amazon QuickSight**
  ...
  * S3 또는 Redshift의 데이터를 기반으로 대시보드 생성
  * 실시간 분석 및 시각화 가능
* **Amazon Redshift**

  * Athena나 Lambda를 통해 처리된 데이터를 로드하여 고급 분석
  * QuickSight와 연동 가능
...
| 단계        | AWS 서비스               | 역할                       |
| --------- | --------------------- | ------------------------ |
| 데이터 수집    | IoT Core              | IoT 장치 데이터 수집            |
| 스트리밍      | Kinesis Data Streams  | 실시간 데이터 스트리밍             |
| 배치 전송     | Kinesis Data Firehose | S3로 데이터 전송, Lambda 연결 가능 |
| 데이터 처리    | AWS Lambda            | 변환, 클렌징, Athena 쿼리 트리거   |
| 저장        | Amazon S3             | 인제스트 버킷 / 보고 버킷          |
| 메시징       | SQS/SNS               | Lambda 트리거용 선택적 큐        |
| 쿼리/분석     | Amazon Athena         | 서버리스 SQL 쿼리 수행           |
| 시각화       | QuickSight            | 대시보드 생성 및 분석             |
| 데이터 웨어하우스 | Redshift              | 고급 분석 및 QuickSight 연동    |

---

## 🔹 핵심 포인트

1. IoT Core → Kinesis → Firehose → S3 → Lambda → Athena → QuickSight/Redshift
   이런 순서로 **실시간 데이터 수집과 분석**이 가능
2. **완전히 서버리스 환경**으로 운영 가능 → 관리 부담 최소화
3. Lambda와 Athena를 활용하면 **자동화된 실시간 데이터 처리** 구현 가능
4. QuickSight와 Redshift를 통해 시각화와 심층 분석이 가능
