...
# 🔹 AWS Lake Formation 쉽게 이해하기

## 1) Lake Formation이란?

* **목적:** 데이터 레이크(Data Lake) 구축

  * 데이터 레이크 = 모든 데이터를 한 곳에 모아 분석할 수 있는 중앙 저장소
* **특징:**

  * 완전 관리형 서버리스 서비스
  * 수개월 걸리던 데이터 레이크 구축 → **몇 일 만에 가능**

---

## 2) Lake Formation이 하는 일

1. **데이터 수집(Discovery)**: 여러 소스에서 데이터를 찾아냄
2. **데이터 정제(Cleansing)**: 중복 제거, 오류 수정
3. **데이터 변환(Transform)**: 분석에 맞게 구조 변경
4. **데이터 적재(Ingest)**: S3 기반 데이터 레이크에 저장
5. **보안 관리(Security)**:

   * 행(Row) 단위, 컬럼(Column) 단위 접근 제어
   * 모든 분석 서비스(Athena, Redshift, EMR, QuickSight 등)에서 동일 적용

> 핵심: **복잡한 데이터 준비와 접근 제어를 자동화**
...
* AWS: S3, RDS, Aurora
* 온프레미스: SQL, NoSQL
* 다른 데이터 레이크: 다양한 구조와 비구조 데이터 통합 가능

> **Blueprint**: S3, RDS 등 데이터 소스를 쉽게 데이터 레이크로 옮길 수 있도록 미리 준비된 템플릿 제공

---

## 4) Lake Formation과 Glue

* **Glue 위에 얹혀진 서비스**

  * 사용자는 Glue를 직접 다루지 않고 Lake Formation만 사용
  * Glue의 ETL, 데이터 카탈로그 기능을 활용
...
* 기존 방식 문제점:
  ...
  * Athena, QuickSight, S3, RDS 각각에서 개별 보안 관리 → 번거로움
* Lake Formation 해결:

  * 데이터 레이크(S3)에 모든 데이터 적재
  * 행/컬럼 단위 접근 제어를 중앙에서 설정
  * 연결된 모든 서비스는 Lake Formation 권한만 따름

> 시험 포인트: **“행/컬럼 단위 보안 + 중앙 집중 권한 관리”**
...
| 기능         | 설명                                         |
| ---------- | ------------------------------------------ |
| 데이터 레이크 생성 | S3 기반 데이터 레이크, 구축 속도 빠름                    |
| 데이터 준비     | 수집, 정제, 변환, 적재 자동화                         |
| 데이터 통합     | 구조/비구조 데이터 통합, Blueprint 지원                |
| 보안         | 행/컬럼 단위 접근 제어, 중앙 집중 관리                    |
| 연계 서비스     | Athena, Redshift, EMR, QuickSight 등 분석 서비스 |

---

### ✅ Key Takeaways

* **Lake Formation = 데이터 레이크 구축 + 데이터 준비 자동화 + 중앙 집중식 보안**
* 행/컬럼 단위 접근 제어로 다양한 분석 서비스에서도 통일된 보안 적용 가능
* 여러 데이터 소스를 한 곳에 모아 빠르게 분석 가능

> 한마디로, **“AWS에서 데이터를 안전하게 모으고, 정제하고, 분석할 수 있도록 도와주는 올인원 서비스”**입니다.
