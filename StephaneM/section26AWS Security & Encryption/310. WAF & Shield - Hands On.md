## 1️⃣ 콘솔에서 처음 보이는 구조부터 이해하자

AWS 콘솔에서 **WAF & Shield**를 검색하면 한 화면에 세 개가 나와요.
...
* **WAF**: 규칙을 만든다
* **Shield**: DDoS 보호 상태를 본다
* **Firewall Manager**: 여러 계정에 규칙을 퍼뜨린다

---

## 2️⃣ WAF Hands-on 전체 흐름 (중요 ⭐)

### WAF를 쓴다는 건 결국 이 한 문장

> **“Web ACL 하나 만들어서 리소스에 붙인다”**

---

## 3️⃣ Step 1. Web ACL 만들기
...
## 5️⃣ Step 3. 룰 추가 (핵심 파트)

### 룰 종류는 두 가지

#### ① Managed Rule Group (실무에서 거의 이것만 씀)
...
* 🕵️ **Anonymous IP 차단** (VPN, 프록시)
* 💣 **Known Bad Inputs** (SQL, XSS)
...
## 6️⃣ 용량(Capacity) 개념 한 번에 정리

* Web ACL 최대 용량: **1500**
* 룰마다 “무게”가 있음
* 너무 많은 룰 넣으면 생성 안 됨

👉 **시험 포인트**지만 실무에선 크게 신경 안 씀

---

## 7️⃣ Step 4. 기본 동작 (Default Action)
...
* 각 룰마다 CloudWatch 메트릭 생성
* “어떤 룰이 얼마나 막았는지” 확인 가능

---

## 9️⃣ IP Set도 따로 관리 가능
...
## 1️⃣1️⃣ Firewall Manager Hands-on 감각

### Firewall Manager는 언제 쓰나?
...
## 1️⃣2️⃣ 세 서비스 Hands-on 관점 정리

| 서비스              | 콘솔에서 하는 일         |
| ---------------- | ----------------- |
| WAF              | Web ACL 만들고 룰 붙이기 |
| Shield           | DDoS 보호 상태 확인     |
| Firewall Manager | 여러 계정에 규칙 강제      |

---

## 1️⃣3️⃣ 전체 흐름을 한 그림으로 정리
...
Firewall Manager는 위 규칙들을
**모든 계정에 자동 배포**

---

## 🔑 최종 핵심 요약 (Hands-on 관점)

* WAF = **Web ACL 만들기**
* Scope 선택에서 CloudFront vs Regional 주의
* Managed Rule Group이 핵심
* 기본 동작은 Allow
* Shield Advanced는 실습 금물
* Firewall Manager는 조직용
