## 1️⃣ KMS Multi-Region Key 한 줄 요약

> **KMS Multi-Region Key는 “같은 암호 키를 여러 리전에 복제해 두는 것”이다.**
> 그래서 **한 리전에서 암호화한 데이터를 다른 리전에서 바로 복호화**할 수 있다.

---

## 2️⃣ 일반 KMS 키 vs Multi-Region 키
...
* 특징:
  ...
  * 🔑 **키 ID가 동일**
  * 🔐 **키 재료(Key Material)가 동일**
  * 키 이름에 `mrk-` 가 붙음
* 결과:

  * 서울에서 암호화 → 도쿄에서 바로 복호화 가능

👉 핵심 차이:
**“같은 열쇠를 여러 나라에 복사해 둔 것”**

---

## 3️⃣ 자동 키 회전도 같이 복제됨

* Primary 키에서 자동 회전 ON →
* 모든 리전의 Replica 키도 **동시에 회전**
...
## 5️⃣ Multi-Region 키를 쓰는 “정확한” 경우

### ✅ 대표적인 3가지

1. **클라이언트 사이드 암호화 (Client-Side Encryption)**
2. **DynamoDB Global Tables**
3. **Aurora Global Database**

---

## 6️⃣ DynamoDB Global Table 예제로 이해하기
...
1. 클라이언트가:
   ...
   * 주민번호(SSN) 같은 **민감 필드만**
   * **Multi-Region KMS 키로 암호화**
2. 암호화된 상태로 DynamoDB에 저장
3. 테이블이 다른 리전으로 자동 복제됨

---

### 🔓 데이터 읽기 흐름 (다른 리전)

1. ap-southeast-2에서 데이터 조회
2. 암호화된 필드 존재
3. **로컬 리전의 KMS Replica 키**로 복호화
4. 빠르고(Low latency) 안전함
...
## 7️⃣ Aurora Global Database도 동일한 개념

* 민감 컬럼만:

  * AWS Encryption SDK + Multi-Region 키로 암호화
* 글로벌 DB 복제
* 각 리전에서:

  * **자기 리전 KMS로 복호화**

👉 DynamoDB와 원리는 완전히 동일
