## 1️⃣ Firewall Manager가 뭐야? (한 줄 요약)

**AWS Firewall Manager는 “여러 AWS 계정의 방화벽 규칙을 한 곳에서 통제하는 중앙 관제 시스템”**이에요.

👉 **직접 공격을 막는 도구가 아니라,
보안 규칙을 ‘관리·강제·자동 적용’하는 관리자 역할**

---

## 2️⃣ 왜 Firewall Manager가 필요해?
...
* 😱 계정마다 설정 다름
* 😱 누군가 새 ALB 만들고 WAF 안 붙임
* 😱 보안 기준 붕괴

👉 **“사람이 실수하지 않게 시스템으로 강제”**해야 함
→ 그게 Firewall Manager

---

## 3️⃣ Firewall Manager의 핵심 개념: Security Policy

### Security Policy란?

> **“우리 회사 공통 보안 규칙 세트”**

이 정책 하나로 아래를 통제할 수 있어요.
...
### Firewall Manager가 하는 일

* 📍 **한 콘솔에서 전체 계정 보안 상태 확인**
* 🧠 규칙을 **조직 전체에 강제**
* 🤖 새 리소스 생성 시 **자동 적용**
...
* Firewall Manager 정책은 **리전 단위**
* 해당 리전의 모든 계정 리소스에 적용
* CloudFront 관련 정책은 글로벌

---

## 7️⃣ WAF / Shield / Firewall Manager 차이 정리 (핵심)

### 이 부분이 시험에 가장 많이 나옴

| 서비스                  | 역할                        |
| -------------------- | ------------------------- |
| **WAF**              | 개별 리소스 보호 (HTTP 규칙 직접 설정) |
| **Shield Advanced**  | DDoS 방어 + 전문가 대응          |
| **Firewall Manager** | 여러 계정에 보안 규칙 강제 & 자동화     |

📌 비유하면:

* **WAF** = 문 앞 경비원
* **Shield** = 폭동 대비 장비
* **Firewall Manager** = 본사 보안 통제실

---

## 8️⃣ Shield Advanced + Firewall Manager 조합
...
## 9️⃣ 언제 Firewall Manager를 써야 할까?
...
> **AWS Firewall Manager는 여러 AWS 계정에 걸쳐
> WAF·Shield·보안 그룹·네트워크 방화벽 규칙을 중앙에서 관리하고 자동 적용하는 보안 통제 서비스다.**
