...
**CIDR은 “IP 주소를 묶어서 범위로 표현하는 방법”**이에요.

예전엔 IP를 반(클래스) 단위로만 나눴는데, CIDR은 **필요한 만큼만 유연하게 나누기** 위해 나온 방식입니다.
...
* 범위에 포함된 IP 중 하나
* 보통 “시작점”처럼 사용됨
* 예: `192.168.0.0`, `10.0.0.0`

### ✅ ② /숫자 (Subnet Mask)

* **앞에서부터 몇 비트가 고정인지**를 의미
* 숫자가 클수록 → 범위가 **작아짐**
* 숫자가 작을수록 → 범위가 **커짐**

---

## 3️⃣ /숫자가 의미하는 것 (직관적으로)
...
> **“이 4덩어리 중 어디까지 고정하고, 어디부터 바꿀 수 있나?”**
> 를 정해주는 역할입니다.
...
| ----- | ------------- | -------- |
| `/32` | 아무것도 못 바꿈     | 1개       |
| `/24` | 마지막 한 덩어리만 바꿈 | 256개     |
| `/16` | 뒤 두 덩어리 바꿈    | 65,536개  |
| `/8`  | 뒤 세 덩어리 바꿈    | 약 1,600만 |
| `/0`  | 전부 바꿈         | 모든 IP    |
...
## 5️⃣ CIDR이 왜 중요한가? (AWS 관점)

AWS에서 CIDR은 거의 모든 네트워크 설정에 등장합니다.

* VPC IP 대역 설정
* Subnet 나누기
* Security Group / NACL 접근 제어
* “어디서 접속 허용할 것인가”를 정의

👉 **CIDR = 네트워크의 지도**라고 생각하면 됩니다.
...
인터넷에서 **직접 접근 불가능**, 내부 네트워크용

IANA가 미리 “사설용”으로 예약해둔 범위입니다.

| 범위               | 특징           |
| ---------------- | ------------ |
| `10.0.0.0/8`     | 대규모 회사, 클라우드 |
| `172.16.0.0/12`  | 중간 규모        |
| `192.168.0.0/16` | 가정용 공유기      |
...
* 전 세계에서 **유일**
* 인터넷에서 직접 접근 가능
* AWS EC2에 붙는 Elastic IP 같은 것

👉 **Private 범위가 아닌 모든 IP는 Public IP**
