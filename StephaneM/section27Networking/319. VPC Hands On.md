...
1. **VPC 생성**
   ...
   * 이름(Tag): `DemoVPC`
   * 기본 목적: 직접 하나씩 설정해보며 학습

2. **IPv4 CIDR 블록 선택**

   * CIDR = 네트워크 안에서 쓸 IP 주소 범위
   * 선택한 범위: `10.0.0.0/16`
...
3. **Tenancy 설정**
   ...
   * **Default(공유 하드웨어)** vs **Dedicated(전용 하드웨어)**
   * Default 사용 → 비용 절약, 일반적인 설정
   * Dedicated는 비싸지만 보안/성능이 중요한 경우 사용 가능
...
* 생성 후 **추가 IPv4 CIDR 블록**을 넣을 수 있음
* 예: `10.1.0.0/16`
  → 기존 범위 뒤에 새로운 범위 추가
* 한 VPC에서 최대 **5개의 IPv4 CIDR 블록** 가능
* IPv6 CIDR 블록도 필요 시 추가 가능
* 이번 실습에서는 **단일 IPv4 CIDR 블록**만 사용

---

## 3️⃣ 핵심 포인트 (Key Takeaways)

| 항목           | 내용                             |
| ------------ | ------------------------------ |
| VPC 이름       | DemoVPC                        |
| IPv4 CIDR 블록 | 10.0.0.0/16 (~65,000 IP)       |
| 최대 CIDR 크기   | /16                            |
| Tenancy      | Default(공유 하드웨어), Dedicated 가능 |
| 추가 CIDR 블록   | 생성 후 최대 5개까지 가능                |
| 자동 생성 리소스    | 메인 라우트 테이블, 메인 네트워크 ACL        |

---

쉽게 말하면,

> **“DemoVPC는 AWS 안에서 내 IP 주소 범위를 가진 독립적인 네트워크”**
> 라고 이해하면 됩니다.

* 첫 단계: **VPC 만들기 + CIDR 지정 + Tenancy 선택**
* 다음 단계: 서브넷, 라우트 테이블, 인터넷 게이트웨이 등 추가 설정
