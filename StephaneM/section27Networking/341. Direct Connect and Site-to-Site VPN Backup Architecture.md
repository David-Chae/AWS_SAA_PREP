...
* **Direct Connect(DX)** = 회사 데이터센터 ↔ AWS VPC **전용선 연결**

  * 장점: 안정적, 빠른 속도, 인터넷 영향 없음
  * 단점: 비용이 비쌈, 연결 장애 시 복구까지 시간 소요

* **Site-to-Site VPN** = 회사 데이터센터 ↔ AWS VPC **인터넷을 통한 암호화 터널**

  * 장점: 설정 쉽고, 인터넷만 있으면 연결 가능
  * 단점: 인터넷 영향 받음 → 속도/지연 불안정

---

## 2️⃣ 왜 둘을 같이 쓰나?

DX는 안정적이지만 **완전히 끊길 수 있음** → 장애 대비 필요

* DX가 다운되면 VPC 접근이 끊김
* 하지만 인터넷은 대부분 살아 있음 → VPN으로 연결 가능

즉, **DX를 주 연결(Primary), VPN을 백업(Secondary)**으로 사용하는 구조
...
회사 데이터센터
    │
    │ Site-to-Site VPN (자동 활성화)
    ▼
AWS VPC (인터넷 경유)
```

* 평소에는 **DX**로 트래픽 흘러감
* DX 장애 발생 시 **VPN**이 자동으로 켜져서 연결 유지

---

## 4️⃣ 핵심 포인트 (시험용)

1. **Primary** = Direct Connect → 안정적, 비용 높음
2. **Backup** = Site-to-Site VPN → 인터넷 경유, 장애 시 자동 활성화
3. 이 구조를 사용하면 **VPC 접근이 끊기지 않음**
4. 시험에서 “비용 효율적이면서도 연속성 보장” 관련 문제 자주 등장

---

한 줄 요약:

> **Direct Connect는 주 연결, Site-to-Site VPN은 인터넷 기반 백업 연결로 구성하여 VPC 연결 연속성을 보장하는 아키텍처**
