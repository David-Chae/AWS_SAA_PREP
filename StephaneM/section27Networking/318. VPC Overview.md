...
* **VPC = Virtual Private Cloud**
  → AWS 안에서 내가 직접 만드는 **사설 네트워크**
* EC2, RDS 같은 AWS 리소스를 이 안에 넣어 네트워크를 관리
* 각 VPC는 서로 **독립적**이라 다른 VPC와 충돌 없이 구성 가능

---

## 2️⃣ 한 리전(region)에서 VPC 개수

* 기본 최대 **5개 VPC** 생성 가능 (AWS 계정별)
* 필요하면 요청해서 **더 늘릴 수 있음**
* 즉, 한 리전 안에서 여러 네트워크를 운영할 수 있음

---

## 3️⃣ VPC의 IP 범위 (CIDR)

* 각 VPC에는 **최대 5개의 CIDR 블록** 할당 가능
* CIDR 범위 크기:

  * 최소 `/28` → **16개 IP**
  * 최대 `/16` → **65,536개 IP**
* **CIDR = 네트워크의 IP 주소 범위를 지정**하는 방법
...
* 사용 가능한 범위:
  ...
  192.168.0.0/16
  ```
* 즉, **인터넷에서 바로 접근 가능한 공인 IP는 VPC에 직접 사용 불가**

---

## 4️⃣ CIDR 선택 시 주의점

* **중요:** 다른 VPC나 사내 네트워크와 **IP 범위가 겹치지 않도록 설정**
* 이유: IP 범위가 겹치면 **라우팅 충돌** 발생 → 서로 통신 불가
* 따라서 미리 **네트워크 설계를 고려**하고 CIDR을 결정해야 함
...
| 항목         | 내용                                                 |
| ---------- | -------------------------------------------------- |
| VPC        | AWS 안에서 내가 만드는 사설 네트워크                             |
| 한 리전 VPC 수 | 기본 5개, 요청 시 증가 가능                                  |
| CIDR 블록 수  | 최대 5개                                              |
| CIDR 블록 크기 | /28(16 IP) ~ /16(65,536 IP)                        |
| IP 범위      | 사설 IPv4만 사용 가능 (10.x.x.x, 172.16.x.x, 192.168.x.x) |
| 주의         | 다른 네트워크와 **IP 범위 겹치지 않게** 설정 필요                    |

---

쉽게 말하면,

> **VPC = 내 AWS 안의 작은 사설 네트워크, CIDR = 그 네트워크 안에서 쓸 IP 범위**
> 라고 이해하면 됩니다.
