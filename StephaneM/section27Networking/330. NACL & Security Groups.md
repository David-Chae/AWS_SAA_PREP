## 1️⃣ Security Group과 NACL, 역할부터 한 줄로
...
> **Subnet 입구를 지키는 “문 앞 경비”**

* Subnet 단위
* 허용 + 차단(Deny) 가능
* 연결 상태를 기억 안 함 (Stateless)

---

## 2️⃣ 인바운드 트래픽 흐름 (밖 → EC2)

외부에서 EC2로 요청이 들어올 때 순서:
...
## 3️⃣ Stateful vs Stateless (핵심 개념)

### ✅ Security Group = Stateful

> “한 번 허용된 연결은 **응답 자동 허용**”

* Inbound 허용 → Outbound 자동 OK
* Outbound 허용 → Inbound 자동 OK

👉 응답 트래픽에 대해 **규칙 안 써도 됨**
...
👉 **왕복 트래픽을 전부 규칙으로 열어야 함**

---

## 4️⃣ 아웃바운드 트래픽 흐름 (EC2 → 밖)

EC2가 `google.com` 접속할 때:
...
NACL Inbound (다시 검사!)
 ↓
Security Group Inbound (자동 허용)
 ↓
EC2
```

📌 여기서 **NACL만 응답도 검사**한다는 게 포인트

---

## 5️⃣ NACL 기본 성질 (시험에 자주 나옴)

### 📍 적용 범위

* **Subnet 하나당 NACL 하나**
* 해당 Subnet의 모든 EC2에 적용
...
* 1 ~ 32766
* **숫자 작을수록 우선순위 높음**
* **첫 번째로 매칭되는 규칙이 바로 적용**

예:

* Rule 100: ALLOW 1.2.3.4
* Rule 200: DENY 1.2.3.4
  👉 **100이 이김 → 허용**

📌 AWS 추천: **100 단위로 규칙 추가**
...
## 6️⃣ Default NACL vs Custom NACL
...
## 8️⃣ 왜 Ephemeral Port가 NACL에서 중요할까?
...
* Outbound: TCP 3306 → DB CIDR 허용

**DB Subnet NACL**

* Inbound: TCP 3306 ← Web CIDR 허용
...
* **Ephemeral Port** 로 응답함!

그래서 반드시 필요:

**DB Subnet NACL**

* Outbound: TCP **Ephemeral Port Range** → Web CIDR

**Web Subnet NACL**

* Inbound: TCP **Ephemeral Port Range** ← DB CIDR
...
## 🔟 Security Group vs NACL 요약표

| 항목             | Security Group | NACL         |
| -------------- | -------------- | ------------ |
| 적용 범위          | EC2            | Subnet       |
| 허용/차단          | Allow만         | Allow + Deny |
| 상태 기억          | Stateful       | Stateless    |
| 응답 트래픽         | 자동 허용          | 규칙 필요        |
| 규칙 평가          | 전체 평가          | 첫 매칭         |
| Ephemeral Port | 신경 ❌           | 필수           |

---

## 1️⃣1️⃣ 시험에서 반드시 기억할 문장

💡 **시험 암기 문장**

* Security Group은 **Stateful**
* NACL은 **Stateless**
* NACL은 **Inbound / Outbound 둘 다 열어야 함**
* Default NACL은 **모두 허용**
* Ephemeral Port는 **NACL에서 중요**
