좋아요, Transit Gateway(TGW) 내용을 쉽게 풀어서 정리해드릴게요. 핵심 개념부터 구조, 장점, 그리고 시험 포인트까지 한눈에 들어오도록 설명합니다.

---

## 1️⃣ 문제 상황: VPC와 VPN, Direct Connect가 많아지면?

AWS에서 VPC가 여러 개, VPN 연결이 여러 개, Direct Connect도 여러 개 생기면 네트워크가 복잡해져요.

* VPC끼리 일일이 피어링 → 관리 어려움
* VPN과 DX 연결 → 매번 따로 설정
  → 연결 구조가 점점 “망”처럼 복잡해짐

---

## 2️⃣ 해결책: **Transit Gateway(TGW)**

**Transit Gateway**는 AWS에서 제공하는 **허브(Hub) 역할**을 하는 서비스입니다.

* 여러 VPC, VPN, Direct Connect를 **중앙에서 연결**
* VPC끼리 개별 피어링 없이 **Transit Gateway를 통해 통신 가능** (Transitive Peering)
* 구조: **허브-앤-스포크(Hub-and-Spoke)**

```
      VPC1
        \
         \
          TGW ---- DX / VPN ---- Data Center
         /
      VPC2
```

* 중앙 허브(TGW)를 통해 모든 VPC와 연결되므로 관리가 단순해짐

---

## 3️⃣ 주요 특징

1. **지역(Resource in Region) 단위**: 기본적으로 한 리전 내에서 사용

   * 필요 시 **Cross-Region Peering**으로 다른 리전 VPC와 연결 가능

2. **계정 공유 가능**: Resource Access Manager(RAM)를 통해 다른 AWS 계정과 공유 가능

3. **경로(Route) 제어 가능**:

   * TGW 내부에 **Route Table**을 만들어 VPC/연결별 통신 허용 여부 조절 가능
   * 보안 강화 가능

4. **IP 멀티캐스트 지원**:

   * TGW는 AWS에서 유일하게 IP 멀티캐스트를 지원

---

## 4️⃣ VPN 성능 향상: ECMP

* 기본 VPN → 단일 VPC 연결, 최대 1.5 Gbps
* TGW + **ECMP(Equal-Cost Multi-Path)** → 여러 VPN 터널을 동시에 사용 가능

  * 예: VPN 2개 연결 → 4개의 터널 → 2.5 Gbps 이상 가능
* 즉, TGW를 쓰면 VPN 성능(Throughput)을 늘릴 수 있음

---

## 5️⃣ Direct Connect 공유

* TGW를 사용하면 **한 개의 Direct Connect**를 여러 VPC, 여러 계정에서 동시에 사용 가능
* 구조:

```
Data Center --- DX --- DX Gateway --- TGW --- VPC1/VPC2 (다른 계정 포함)
```
...
| 특징                   | 장점                             |
| -------------------- | ------------------------------ |
| Transitive Peering   | VPC끼리 직접 피어링 필요 없음             |
| Hub-and-Spoke        | 복잡한 네트워크 단순화                   |
| Route Table 제어       | VPC 간 통신 세밀하게 제어 가능            |
| ECMP 지원              | VPN 대역폭 확장 가능                  |
| DX 공유                | 여러 계정/VPC에서 Direct Connect 재사용 |
| Cross-Region Peering | 여러 리전 VPC 연결 가능                |
| IP 멀티캐스트 지원          | 멀티캐스트 트래픽 사용 가능                |

---

## 7️⃣ 시험 포인트

1. **TGW = 허브 역할, VPC 트랜짓(peering) 지원**
2. **VPN ECMP → Throughput 확장 가능, 단일 VPN은 1.5 Gbps 제한**
3. **Direct Connect 공유 가능** → 비용 효율적
4. **Route Table로 통신 제어 가능**
5. **IP 멀티캐스트 시 TGW 사용**
