...
> **Public Subnet에 NAT Instance를 두고,
> Private Subnet의 EC2가 인터넷에 접속하도록 만든다**

---

## 1️⃣ NAT Instance 생성하기 (AMI 선택)

### 🔍 왜 AMI를 따로 찾나?

* NAT Instance는 **일반 EC2가 아니라**
* **NAT 기능이 미리 설정된 특수 AMI**를 사용해야 함

### 🔹 실습 순서

1. **Launch Instance**
2. AMI 검색에서 `NAT` 입력
3. **Community AMI** 탭 선택
   (공식 Amazon 제공 NAT AMI는 여기 있음)
4. 조건 확인:

   * 제공자: AWS
   * 아키텍처: **x86_64**
   * 비교적 최근 날짜 (예: 2022)
   * 이름 예: `Amazon AMI VPC NAT 2018`

📌 NAT Instance는 이제 거의 안 쓰이지만
👉 **개념·실습용으로는 충분**
...
## 3️⃣ 네트워크 & 보안 그룹 설정 (중요)

### ❓ 왜 보안 그룹이 중요?

NAT Instance는 **Private EC2들의 트래픽을 대신 받아서 인터넷으로 보내는 중계자**
👉 허용 규칙이 없으면 트래픽이 막힘
...
## 5️⃣ Source / Destination Check 비활성화 (핵심)
...
1. 인스턴스 **Stop**
2. 설정에서 Source/Destination Check 끄기
3. 저장 후 다시 Start

---

## 6️⃣ Private Subnet 라우팅 수정

### 🎯 목적

> “인터넷으로 가는 트래픽은 전부 NAT Instance로 보내라”

### 🛣 Route Table 수정

Private Subnet에 연결된 Route Table에 추가:
...
## 7️⃣ Private EC2에서 인터넷 되는지 확인

### 🔐 접속 구조

Private EC2는 Public IP가 없으므로:

```
내 PC → Bastion Host → Private EC2
```

1. Bastion Host 접속
2. 거기서 Private EC2로 SSH
...
* NAT Instance 보안 그룹에 **ICMP 허용 안 됨**

---

## 8️⃣ ICMP 허용해서 ping 살리기
...
* SSH는:
  ...
  * Bastion Host를 통해서만 가능
* 인터넷 나가기만 허용됨 (Outbound 전용)
...
1. **Public Subnet에 NAT Instance**
2. **Source/Destination Check 비활성화**
3. **Private Route Table → NAT Instance**
4. **Security Group**
...
👉 다음 단계에서 NAT Gateway를 배우면
“아 왜 이걸 쓰는지” 바로 이해될 거야.
