이번에는 **Bastion Host를 실제로 어떻게 써서 Private EC2에 접속하는지**를
👉 **“왜 → 무엇을 → 어떻게 → 주의점”** 순서로 아주 쉽게 풀어 설명할게요.

---

## 한 줄 요약

**Public Subnet의 Bastion Host를 ‘중계 서버’로 삼아,
Private Subnet의 EC2에 SSH로 들어가는 실습이다.**
...
* ✅ Public Subnet에 EC2 하나 있음 → **Bastion Host**
* ❌ Private Subnet에 EC2는 아직 없음

### 목표

* Private Subnet에 EC2 생성
* **인터넷에 직접 노출하지 않고**
* Bastion Host를 통해서만 SSH 접속

---

## 2️⃣ 왜 Key Pair(키 파일)가 필요한가?

SSH는 **열쇠 방식 로그인**임

* Key Pair = 🔑 집 열쇠
* Public Key → EC2에 저장됨
* Private Key (.pem) → 내가 들고 있음
...
* Key Pair: demo-key-pair.pem

---

## 4️⃣ 보안 그룹 설정 (가장 중요한 부분)

### ❌ 잘못된 설정

* SSH를 0.0.0.0/0에 허용
  → Private Subnet 의미 없음

### ✅ 올바른 설정

* Private EC2용 보안 그룹 (privateSG)
* SSH(22번 포트) 허용
* **대상: Bastion Host의 보안 그룹만**

📌 의미:

> “이 EC2는 Bastion Host에서 온 SSH만 받겠다”

---

## 5️⃣ 왜 Private EC2에 직접 접속이 안 될까?
...
* Bastion Host 안에는 **키 파일이 없음**

해결:

* Bastion Host에 **키 파일을 직접 만들어야 함**

---

## 8️⃣ Bastion Host에 키 파일 만들기

### 작업 순서

1. 텍스트 편집기 실행 (vi / nano)
2. `demo-key-pair.pem` 파일 생성
3. **로컬에서 다운받았던 키 내용 그대로 붙여넣기**
4. 저장 후 종료
...
* `<private-ip>` → Private EC2의 사설 IP

👉 성공하면:

* Amazon Linux 로그인 화면 등장
* **Private Subnet EC2 접속 완료 🎉**
...
### ❌ invalid format / passphrase error
...
> **Bastion Host 실습의 본질은
> “Private EC2를 외부에 노출하지 않고도 관리할 수 있게 만드는 것”이다.**

다음 단계는 자연스럽게
👉 **NAT Gateway로 Private EC2 인터넷 열어주기** 입니다.
