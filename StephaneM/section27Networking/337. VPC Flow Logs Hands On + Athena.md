아래 내용을 **“실제로 VPC Flow Logs를 만들고, S3에 쌓아서 Athena로 분석하는 전체 흐름”** 기준으로 아주 쉽게 정리해줄게요.

---

# VPC Flow Logs Hands-On + Athena 한 번에 이해하기

## 1️⃣ 오늘 실습의 목표

이 실습의 목적은 딱 3가지입니다.

1. **VPC 네트워크 트래픽을 기록하고**
2. **그 로그를 S3와 CloudWatch에 저장한 뒤**
3. **Athena로 공격/차단 트래픽을 SQL로 분석하는 것**

즉,
👉 *“누가 우리 VPC를 두드리고 있는지 눈으로 확인하는 과정”*입니다.

---

## 2️⃣ VPC Flow Log 생성 (기본 개념)

### 🔹 어디서 생성?

* **VPC → Flow Logs** 메뉴
* Demo VPC에 Flow Log 생성
...
## 3️⃣ Aggregation Interval (비용 포인트)

* **1분**: 빠르게 로그 생성 (데모용)
* **10분**: 로그 수 적음 → 비용 절감 (실무 추천)

👉 로그가 많을수록 **S3 / CloudWatch 비용 증가**

---

## 4️⃣ Flow Logs를 S3로 보내기

### 🔹 S3 버킷 생성

* 이름: `demo-xxxx-vpc-flow-logs`
* **VPC와 같은 리전**

### 🔹 왜 ARN이 필요한가?

* VPC Flow Logs 서비스가
  → **이 S3 버킷에 로그를 쓸 수 있도록 허용**해야 함

👉 AWS가 **버킷 정책을 자동으로 생성**해줌
(직접 쓸 필요 없음)

---

## 5️⃣ 두 번째 Flow Log (CloudWatch Logs)

이번엔 **실시간 분석용**입니다.

### 🔹 설정 요약

* 이름: `DemoFlowLog-CloudWatch`
* 트래픽: ALL
* 대상: CloudWatch Logs

---

## 6️⃣ IAM Role 설정 (시험 포인트)

CloudWatch로 보내려면 **IAM Role 필수**
...
## 7️⃣ CloudWatch Log Group 생성

* 이름: `VPCFlowLogs`
* 보관 기간: 1일 (데모용)

👉 이제 **S3 + CloudWatch** 두 군데에 로그 쌓임
...
* **ENI(네트워크 카드) 단위 로그 스트림**
* Bastion Host ENI ID와 매칭
...
## 9️⃣ CloudWatch vs S3, 언제 쓰나?

| 목적              | 추천              |
| --------------- | --------------- |
| 실시간 탐지 / 알람     | CloudWatch Logs |
| 대규모 분석 / 과거 데이터 | S3 + Athena     |

---

## 🔍 Athena로 Flow Logs 분석하기

### 1️⃣ Athena 결과 저장용 S3 버킷

* 예: `demo-athena-results`
* Athena 설정에서 지정
...
AWS에서 제공하는 **VPC Flow Logs용 CREATE TABLE 문** 사용

* 로그 위치:

```
s3://버킷명/AWSLogs/account-id/vpcflowlogs/region/
```
...
### 3️⃣ Partition 추가 (날짜별)

```sql
ALTER TABLE vpc_flow_logs
ADD PARTITION (year='2026', month='01', day='01');
```
...
* VPC Flow Logs = 네트워크 트래픽 기록기
* CloudWatch → 실시간 보안 감시
* S3 + Athena → 공격 패턴 분석
* REJECT 로그 = 문제의 핵심
* 시험에서:
