## 1️⃣ AWS Control Tower란?

* **목적**: 여러 AWS 계정을 쉽고 안전하게 만들고, 규칙(컴플라이언스)을 지키면서 관리하도록 도와주는 서비스
* **기반**: AWS Organizations
  → Control Tower가 여러 계정을 자동으로 만들고 관리

---

## 2️⃣ Control Tower 사용 장점

* 클릭 몇 번으로 **환경 세팅 자동화**
* 원하는 **설정 미리 구성 가능**
* **규칙(Guardrails) 적용** → 정책 관리 자동화
* **규칙 위반 감지 및 자동 수정 가능**
* **대시보드 제공** → 모든 계정의 규정 준수 상태 모니터링

💡 쉽게 말하면:

> “여러 AWS 계정을 만들고, 안전하게 운영하도록 지켜주는 자동 관리 서비스”

---

## 3️⃣ Guardrails(규칙) 이해

Control Tower는 **계정 운영 규칙**을 적용하는데, 이를 Guardrail이라고 부릅니다.

### 3-1️⃣ Preventive Guardrails (예방 규칙)

* 계정이 **허용되지 않은 행동을 못하게 제한**
* 구현 방법: **Service Control Policies (SCPs)**
* 예시:

  * 특정 AWS 리전만 사용 가능 → us-east-1, eu-west-2
* 특징: **행동 자체를 막음**

### 3-2️⃣ Detective Guardrails (감시 규칙)
...
| 항목         | 설명                                           |
| ---------- | -------------------------------------------- |
| 목적         | 다계정 AWS 환경 생성 + 보안/규정 준수 관리                  |
| 기반 서비스     | AWS Organizations                            |
| 자동화        | 계정 생성, 설정 적용, 규칙 적용                          |
| Guardrails | Preventive: 행동 제한 / Detective: 감시 + 자동 조치 가능 |
| 모니터링       | Control Tower 대시보드로 모든 계정 상태 확인              |

💡 한 줄 요약:

> **Control Tower = 여러 계정을 쉽고 안전하게 만들고, 규칙을 적용하며 자동으로 관리하는 서비스**
