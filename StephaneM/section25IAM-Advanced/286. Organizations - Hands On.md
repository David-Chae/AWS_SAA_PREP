...
* 여러 AWS 계정을 한 곳에서 관리할 수 있게 해주는 글로벌 서비스.
* “관리 계정(Management/Master Account)”이 중심이 되고, “자식 계정(Child Account)”들을 조직 단위로 묶어 관리함.

---

### 2. 계정 준비

* 실습용으로 새로운 AWS 계정 2개를 만듭니다.

  1. 관리 계정(Master)
  2. 자식 계정(Child)
* 메인 계정을 쓰지 않고 새 계정을 만들어 실습하는 것이 안전합니다.

---

### 3. Organization 만들기

* 관리 계정에서 Organization 생성 → 기본 단위는 **Root**.
* Root 아래에는 관리 계정이 자동으로 포함됨.

---

### 4. 두 번째 계정 추가

* 새 계정을 만들거나 기존 계정을 초대할 수 있음.
* 초대 시 이메일을 입력 → 자식 계정에서 초대를 수락 → Organization에 등록됨.
* 등록된 자식 계정은 관리 계정의 제어를 받게 됨.

---

### 5. 계정 구조 관리: OU (Organizational Units)

* **OU란?** 계정을 그룹화해서 구조적으로 관리할 수 있는 단위.
* 예시:
  ...
  * Prod (운영 계정)
    ...
    * Prod 안에 Finance, HR 같은 부서별 OU 생성 가능
* OU는 계정을 이동시키거나 정책을 적용할 때 기준이 됨.
* 관리 계정은 보통 Root 아래에 그대로 둠.

---

### 6. 정책 관리: Service Control Policies (SCPs)

* SCP는 계정이나 OU의 **권한을 제한하는 정책**.
* 예: Finance OU에 S3 접근 금지 정책(DenyAccessS3) 적용 → Finance OU 안의 모든 계정은 S3 접근 불가.
* 상위 OU 정책은 하위 OU/계정에 **상속**됨.
* SCP는 루트 계정 사용자도 영향을 받음 → 강력한 권한 제어 가능.

---

### 7. 정책 적용 및 테스트

1. Finance OU에 DenyAccessS3 정책 적용.
2. 자식 계정에서 S3 접근 시도 → 접근 불가 확인.
3. OU 단위 정책 상속으로 계정 권한 통제 가능.

---

### 8. 핵심 요약

* **Organization 생성:** 여러 계정을 한 곳에서 관리.
* **OU 구조:** 계정을 Dev, Test, Prod 등으로 나누고 필요시 부서별 OU 추가.
* **SCP 적용:** OU/계정 권한 제한 → 상속 구조로 효율적 관리.
* **검증:** 자식 계정에서 권한 제한이 실제로 적용되는지 테스트.

---

💡 **쉽게 기억하는 방법:**

> 조직 → OU → 계정 → 정책(SCP) → 권한 제한, 상속

즉, AWS Organizations는 회사 조직도를 그대로 AWS 계정에 적용한다고 생각하면 이해가 쉬워요.
