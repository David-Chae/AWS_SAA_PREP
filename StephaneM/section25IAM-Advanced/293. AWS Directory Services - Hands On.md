## 1️⃣ Directory Services 콘솔에서 볼 수 있는 옵션

* 총 4가지 옵션이 있지만, **Amazon Cognito User Pool은 별도 서비스** → Directory Services 목록에서 제외
* 실제 주요 옵션 3가지:
...
## 2️⃣ AWS Managed Microsoft AD

* AWS에서 **Active Directory 환경을 생성**
* AWS Cloud와 on-prem AD **신뢰 관계(Trust)** 가능 → 사용자 공유 가능
* **에디션 선택 가능**:

  * **Standard Edition**: 최대 30,000 객체
  * **Enterprise Edition**: 최대 500,000 객체
* **MFA 지원**
* 시험 포인트: **AWS와 on-prem AD 통합 가능 + MFA 지원**
...
* **독립형(Standalone) 디렉터리**
* Active Directory 호환 API 제공
* **on-prem AD와 연결 불가**
* AWS 환경만 단독으로 사용할 때 적합
* 시험 포인트: **Standalone, on-prem 연동 불가**

---

## 4️⃣ AD Connector

* **프록시 역할** → 인증 요청을 **on-prem AD로 전달**
* 사용자 관리: **on-prem AD에서만**
* **용량 제한**:

  * 소형 커넥터: 최대 500명
  * 대형 커넥터: 최대 5,000명
* 시험 포인트: **Proxy + on-prem AD 사용, 사용자 제한 존재**

---

## 5️⃣ 핵심 비교표

| 서비스                      | on-prem AD 연결 | 사용자 관리     | MFA | 특징                                |
| ------------------------ | ------------- | ---------- | --- | --------------------------------- |
| AWS Managed Microsoft AD | 가능 (Trust)    | AWS 내      | 지원  | Standard/Enterprise, AWS AD 생성 가능 |
| Simple AD                | 불가            | AWS 내      | 불가  | Standalone, AD API 호환             |
| AD Connector             | 프록시           | on-prem AD | 지원  | 요청 전달만, 사용자 관리 불가, 500~5,000명     |

---

💡 한 줄 요약:

> **Managed Microsoft AD = AWS에서 AD 생성 + on-prem 연결 가능, Simple AD = 단독 AD, AD Connector = on-prem AD 프록시**
