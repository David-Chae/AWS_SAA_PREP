...
* IAM 조건은 **정책에 특정 조건을 추가**해서 권한을 더 세밀하게 제어하는 기능.
* 적용 가능한 정책:

  * IAM 사용자 정책
  * 리소스 정책 (예: S3 버킷)
  * 엔드포인트 정책 등 AWS IAM 내 거의 모든 정책
...
* 특정 IP에서만 접근을 허용하거나 거부할 때 사용.
* 예시: 회사 내부 네트워크 IP만 AWS 리소스 접근 허용 → 외부 접근 차단

### (2) `aws:RequestedRegion`

* 특정 AWS 리전에서만 작업을 허용하거나 거부.
* 예시: EC2, RDS, DynamoDB 접근을 `eu-central-1`, `eu-west-1`로 제한

### (3) `ec2:ResourceTag` & `aws:PrincipalTag`

* **ResourceTag:** 리소스(예: EC2 인스턴스)에 붙은 태그 기반 제어

  * 예: `Project=DataAnalytics` 태그가 있는 EC2만 시작/중지 가능
* **PrincipalTag:** 요청하는 사용자(IAM)의 태그 기반 제어

  * 예: `Department=Data` 태그가 있는 사용자만 특정 작업 수행 가능

### (4) `aws:MultiFactorAuthPresent`

* MFA(2단계 인증) 여부 확인
* 예: EC2 인스턴스 종료/삭제는 MFA를 완료한 사용자만 가능

### (5) S3 Bucket Policy 조건

* **Bucket-level 권한:** 버킷 자체 권한 (예: ListBucket)

  * ARN 예시: `arn:aws:s3:::test`
* **Object-level 권한:** 버킷 안 객체 권한 (예: GetObject, PutObject)
...
* 특정 AWS Organization에 속한 계정만 접근 가능
* 예시: S3 PutObject/GetObject를 조직 계정만 허용 → 외부 계정 접근 차단

---

## 3. 핵심 요약

* IAM 조건을 사용하면 **권한을 세밀하게 통제**할 수 있음.
* 주요 활용:

  * IP, 리전, MFA, 태그, 조직 단위 등 다양한 기준으로 접근 제한
  * S3 버킷과 객체 권한 구분하여 안전한 접근 제어
* 즉, 조건을 잘 활용하면 단순 권한 허용/거부보다 훨씬 강력하고 안전한 보안 정책 설계 가능

---

💡 **쉽게 기억하는 방법:**

> IAM 조건 = “권한을 줄 때 추가하는 ‘만약 ~라면’ 규칙”
> → IP, 리전, 태그, MFA, 조직 등 다양한 기준으로 권한 제한 가능
