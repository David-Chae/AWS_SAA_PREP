이 내용은 AWS의 **CloudFront(CDN)**를 웹사이트의 실제 서버(원본 서버, Origin)인 **ALB(Application Load Balancer)**나 **EC2 인스턴스**와 연결하는 방법에 대한 설명입니다.

핵심은 **더 안전하고 편리한 새로운 방식(VPC Origin)**이 기존의 복잡하고 위험했던 방식(Public Network 사용)을 대체했다는 것입니다.

---

## 💡 CloudFront와 서버 연결의 두 가지 방법

### 1. 🌟 최신/권장 방식: VPC Origin 사용 (가장 좋음)

이 방식은 웹사이트의 서버를 **인터넷에 노출시키지 않고** CloudFront와 안전하게 연결하는 방법입니다.

* **서버 위치:** 여러분의 서버(ALB, NLB 또는 EC2 인스턴스)는 AWS의 사설망인 **VPC(Virtual Private Cloud) 내의 프라이빗 서브넷**에 숨겨져 있습니다. (인터넷에서 직접 접근 불가)
* **작동 방식:**
    * 사용자는 전 세계 CloudFront 배달 거점(Edge Location)에 접속합니다.
    * CloudFront는 **VPC Origin**이라는 특수 연결 통로를 통해 **직접** 여러분의 프라이빗 서버로 트래픽을 보냅니다. 
* **장점:**
    * **최고 보안:** 서버 자체가 인터넷에 공개되지 않으므로, 해킹 위험이 크게 줄어듭니다.
    * **간편한 설정:** 복잡하게 IP 주소를 관리할 필요가 없습니다.

---

### 2. ❌ 기존 방식: 퍼블릭 네트워크 사용 (복잡하고 위험함)

VPC Origin 기능이 나오기 전에는 서버를 인터넷에 일부 노출시켜야만 CloudFront와 연결할 수 있었습니다.

* **서버 위치:** 서버(ALB 또는 EC2)는 **퍼블릭(공개) 네트워크**에 있어야 했습니다.
* **작동 방식:**
    1.  서버의 **보안 그룹**을 설정합니다.
    2.  서버가 **오직 CloudFront의 공개 IP 주소 목록**에서 오는 트래픽만 허용하도록 수동으로 설정해야 했습니다.
* **단점:**
    * **복잡함:** CloudFront의 IP 주소는 수시로 바뀔 수 있어서, 관리자가 **이 목록을 주기적으로 확인하고 수동으로 업데이트**해야 하는 번거로움이 있었습니다.
    * **보안 위험:** 만약 관리자가 실수로 보안 그룹 설정을 잘못하면, 서버가 CloudFront뿐만 아니라 **전 세계 모든 사람에게 공개될 위험**이 있었습니다.

---

## 🔑 핵심 요약 (Key Takeaways)

| 특징 | VPC Origin 사용 (새로운 방식) | 퍼블릭 네트워크 사용 (이전 방식) |
| :--- | :--- | :--- |
| **서버 노출 여부** | **프라이빗** (인터넷에 노출되지 않음) | **퍼블릭** (인터넷에 노출되었으나 IP로 제한) |
| **보안 수준** | **높음** (가장 안전함) | **낮음** (설정 실수 위험 및 IP 관리 복잡) |
| **설정 편의성** | **매우 간편함** | **복잡함** (수동 IP 목록 관리 필요) |

**결론적으로,** CloudFront를 여러분의 백엔드 서버에 연결할 때는 **VPC Origin** 기능을 사용하는 것이 **훨씬 더 안전하고** 설정하기 간편한 **현대적인 모범 사례**입니다.
