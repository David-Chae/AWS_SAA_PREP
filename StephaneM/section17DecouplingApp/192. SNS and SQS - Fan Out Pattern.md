## 1. SNS + SQS Fan-Out 패턴이란?

**핵심 아이디어:**
하나의 메시지를 여러 SQS 큐로 효율적으로 보내는 방법입니다.

* 일반적으로 각 SQS 큐에 메시지를 직접 보내면:

  * 애플리케이션이 메시지 전송 중에 멈추거나
  * 전송 실패가 발생하거나
  * 큐가 늘어날 때 관리가 복잡해지는 문제가 있습니다.

* **Fan-Out 패턴:**

  1. 메시지를 **SNS Topic**에 한 번만 보냅니다.
  2. 여러 **SQS 큐**를 이 Topic에 구독(subscribe)시킵니다.
  3. 그러면 모든 큐가 메시지를 독립적으로 받을 수 있습니다.

**예시:**

* 구매 서비스가 주문 정보를 보내고 싶음
* 사기 탐지 서비스와 배송 서비스가 각자의 SQS 큐를 가지고 있음
* 구매 서비스는 SNS Topic에 한 번 메시지를 게시
* 두 서비스의 SQS 큐가 Topic을 구독 → 두 큐가 각각 메시지를 받음

→ **결과:** 시스템 간 완전히 분리(decoupled), 데이터 손실 없음, 큐 추가도 자유롭게 가능

---

## 2. S3 이벤트와 Fan-Out

* S3 이벤트는 규칙이 제한적이라, 같은 이벤트를 여러 큐로 보내려면 **Fan-Out 패턴** 사용이 필요합니다.
* 과정:

  1. S3 객체 생성 → 이벤트 발생
  2. 이벤트를 SNS Topic에 전송
  3. 여러 SQS 큐나 이메일, Lambda 함수가 Topic 구독 → 모두 이벤트 수신

또 다른 방법으로는 SNS → Kinesis Data Firehose → S3로 직접 데이터를 전송할 수도 있어요.

* 이렇게 하면 SNS 메시지를 S3 같은 저장소로 바로 남길 수 있어 **확장성 있는 메시지 처리** 가능

---

## 3. FIFO (First-In-First-Out) 토픽

* 메시지 순서를 유지하면서 Fan-Out이 필요할 때 사용
* SNS FIFO Topic → 구독자 SQS FIFO 큐로 메시지 순서 보장
* 특징:

  * **메시지 그룹 ID**로 순서 유지
  * **중복 제거** 가능 (deduplication ID 사용)

**예시:** 구매 서비스가 순서대로 주문 데이터를 보낼 때,

* Fraud 서비스와 Shipping 서비스가 FIFO 큐를 구독 → 순서대로 안전하게 처리 가능

---

## 4. 메시지 필터링

* SNS는 구독자별로 **필터 정책**을 적용 가능
* JSON 형식으로 메시지 조건 지정
* 예시:

  * 주문 상태(state)가 `Placed`인 메시지만 특정 SQS 큐로 전달
  * `Canceled` 메시지는 다른 큐로 전달
* 이메일, Lambda 등 다른 구독자에도 필터 적용 가능

→ **결과:** 한 Topic에서 다양한 조건별로 메시지를 선택적으로 분배 가능

---

## 5. 정리 (Key Takeaways)

1. **Fan-Out 패턴:** SNS Topic → 여러 SQS 큐
2. **장점:** 완전 분리, 데이터 손실 없음, 지연 처리 가능, 재시도 가능
3. **확장성:** SQS, Lambda, 이메일, Kinesis Data Firehose 등 다양한 구독자 지원
4. **메시지 필터링:** 조건별로 구독자에게 선택적 전달 가능

---

간단히 말하면, **SNS가 메시지를 한 번만 보내면, 여러 큐/서비스가 동시에 받고, 순서 유지/필터링까지 가능하도록 만들어주는 패턴**이에요.
