...
* 소프트웨어 업데이트를 **EC2 인스턴스**에서 배포
* 많은 사용자가 동시에 업데이트를 받으려 함 → **EC2 CPU 사용량 증가, 네트워크 부담, 비용 상승**
* 기존 애플리케이션 구조는 바꾸지 않고 **성능과 비용 최적화** 필요
...
* **Elastic Load Balancer(ELB) + Auto Scaling Group(ASG)**
  ...
  * 여러 가용 영역(AZ)에서 M5 EC2 인스턴스가 실행
* **Amazon EFS**에 소프트웨어 업데이트 파일 저장 → 모든 EC2가 접근

> 문제: 모든 사용자가 EC2에서 직접 파일 다운로드 → 서버와 네트워크 과부하 발생

---

## 3. **해결 방법**

* **Amazon CloudFront**를 기존 구조 위에 추가

  * CloudFront = 글로벌 **콘텐츠 전송망(CDN)**
  * EC2에서 업데이트 파일을 가져와 **엣지 서버(Edge Location)에 캐싱**
  * 파일이 정적(static)이라 캐시 효과가 큼
* 장점:

  * EC2의 CPU 사용량 감소
  * Auto Scaling Group 덜 확장 → 비용 절감
  * 네트워크 비용 절감
  * 전 세계 사용자에게 빠른 다운로드 가능
* 기존 애플리케이션 구조 **변경 불필요**
...
1. **문제:** EC2에서 직접 업데이트 파일 제공 → 고비용, 과부하
2. **해결:** CloudFront로 캐싱 → Edge 서버에서 배포
3. **효과:**

   * EC2 부담 감소 → 비용 절감
   * 글로벌 사용자 빠른 다운로드
   * 아키텍처 변경 불필요
4. **핵심 포인트:** 가장 단순한 해결책이 종종 최고의 방법

---

쉽게 말하면, **기존 서버는 그대로 두고 CloudFront가 전 세계 사용자에게 업데이트를 대신 배포**하게 만들어서 비용과 성능 문제를 해결하는 방법입니다.
