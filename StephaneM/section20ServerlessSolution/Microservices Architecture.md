...
* 하나의 거대한 애플리케이션 대신, **작고 독립적인 서비스들**로 나누어 개발하는 방식.
* 각 서비스는 **독자적인 코드, 데이터베이스, 배포 환경**을 가질 수 있음.
* 서비스 간 통신은 주로 **REST API**로 이루어짐.
* 장점:

  * 각 서비스별로 **개발 속도 빠름**
  * 필요할 때 **독립적으로 확장 가능**
  * 서비스별로 **코드 관리와 배포 분리 가능**
...
1. **첫 번째 서비스**
   ...
   * 사용자 요청 → **HTTPS → Elastic Load Balancer(ELB) → ECS(Docker 컨테이너) → DynamoDB**
   * URL: `service1.example.com`
   * DNS: **Route 53**에서 주소를 해결함

2. **두 번째 서비스**

   * 서버리스 구조 예시
   * **Lambda → ElastiCache** 사용
   * 필요하면 첫 번째 서비스 ELB 호출

3. **세 번째 서비스**

   * EC2 + RDS (전통적 아키텍처)
   * 첫 번째, 두 번째 서비스 호출 필요
   * URL: `service3.example.com`

> 즉, 각 서비스는 **서로 다른 구조와 기술 스택**을 사용할 수 있음.
...
1. **동기(Synchronous)**
   ...
   * 다른 서비스에 즉시 요청 → 바로 응답 필요
   * 예: HTTPS 호출, API Gateway, ELB
2. **비동기(Asynchronous)**

   * 요청 → 메시지 큐에 넣고 나중에 처리
   * 예: **SQS, SNS, Kinesis, Lambda 트리거, S3 이벤트**
   * 즉시 응답이 필요하지 않음
...
* 새로운 서비스 생성 시 **오버헤드 발생**
* 서버 최적화, 자원 활용 어려움
* 여러 버전 동시 운영의 복잡성
* 클라이언트 코드가 여러 서비스와 통합 필요 → 복잡함

> 서버리스 패턴을 활용하면 일부 단점을 완화 가능
>
> * API Gateway + Lambda → 자동 확장, 사용한 만큼만 비용 지불
> * Swagger 등으로 클라이언트 SDK 생성 → 통합 단순화

---

## 6. **핵심 요약**

* **Microservices = 독립 서비스들의 집합**
* **각 서비스는 다른 기술 스택 가능**
* **동기/비동기 통신** 패턴 선택 가능
* 장점: 독립 확장, 빠른 개발, 기술 자유도
* 단점: 관리 복잡성, 서버 활용 최적화 어려움
* 서버리스 패턴 활용 시 일부 복잡성 감소 가능
