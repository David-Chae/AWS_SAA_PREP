...
* 대부분 **읽기 전용 블로그** → 글을 자주 읽고, 새 글은 하루 1번~주 1번 정도만 작성.
* 방문자가 전 세계에 있음 → **글로벌 확장성 필요**.
* 대부분 **정적 콘텐츠** (HTML, CSS, 이미지)이고, 일부는 **REST API**.
* 새 사용자가 구독하면 **환영 이메일 발송**, 사진 업로드 시 **썸네일 생성** 필요.
* 모든 것을 **서버리스**로 구현.
...
* 정적 파일은 **Amazon S3**에 저장.
* S3는 지역(region)별이므로, 전 세계에 빠르게 전달하려면 **Amazon CloudFront (CDN)** 사용.

  * CloudFront는 엣지 서버에 캐시를 저장 → 사용자와 가까운 서버에서 빠르게 제공.
* **보안**: S3 직접 접근 막고, CloudFront만 접근 가능하게 설정 → Origin Access Control(OAC) + 버킷 정책.

---

## 3. **서버리스 REST API**

* 일부 동적 기능은 **REST API** 필요.
* 모바일/웹 클라이언트 → API Gateway → Lambda → DynamoDB
* 읽기 요청이 많으면 **DynamoDB DAX** 캐시 사용 → 성능 향상.
* 글로벌 사용자 지원 → **DynamoDB Global Tables** 사용 → 세계 각지에서 지연시간 최소화.

---

## 4. **환영 이메일 발송**

* 사용자가 구독 → DynamoDB에 기록 → **DynamoDB Streams** 발생 → Lambda 함수 실행 → **Amazon SES**로 이메일 발송.
* 서버를 직접 운영할 필요 없음, 사용량에 따라 자동 확장.

---

## 5. **이미지 썸네일 자동 생성**

* 사용자가 사진 업로드 → S3 이벤트 트리거 → Lambda 실행 → 썸네일 생성 → S3에 저장.
* S3 → Lambda, SQS, SNS 등 다양한 이벤트 연동 가능 → 서버 없이 이벤트 기반 처리 가능.

---

## 6. **전체 아키텍처 흐름**

1. 정적 블로그: **S3 → CloudFront → 사용자**
2. REST API: **API Gateway → Lambda → DynamoDB (DAX, Global Tables)**
3. 이벤트 기반 기능:

   * 구독 이메일: DynamoDB Streams → Lambda → SES
   * 이미지 썸네일: S3 이벤트 → Lambda → S3

---

## 7. **핵심 장점**

* **서버리스** → 서버 관리 불필요, 자동 확장.
* **글로벌 콘텐츠 제공** → CloudFront + S3.
* **고성능 REST API** → Lambda + DynamoDB + 캐시.
* **이벤트 기반 자동 처리** → 구독 이메일, 이미지 처리 모두 서버 없이 가능.
