# 🔥 Lambda Concurrency — Hands-On 쉽게 이해하기

AWS Lambda에는 “동시에 몇 개까지 실행할 수 있는가?”를 관리하는 **동시성(Concurrency)** 설정이 있어.
이걸 잘 모르면 갑자기 함수가 안 돌아가고 에러가 터질 수 있어.

---

# 1️⃣ 기본 구조: "전체 1000개를 나눠 쓰는 구조"

AWS 계정에는 기본적으로 **1000개의 동시 실행 슬롯**이 있음.

* 모든 Lambda 함수가 이 1000개를 **공유**함.
...
총합이 1000을 넘으면 → 남은 호출들은 전부 **Throttling(차단)** 당함.

---

# 2️⃣ Reserved concurrency = 특정 함수만을 위한 “전용 좌석”

특정 Lambda 함수에게 “전용 슬롯”을 줄 수도 있음.
이걸 **Reserved Concurrency(예약 동시성)** 라고 함.
...
이렇게 하면 중요한 함수가 다른 함수 때문에 막히는 일을 막을 수 있음.

---

# 3️⃣ Reserved concurrency = 0 → 항상 막힘 (테스트용)

만약 **Reserved concurrency = 0** 으로 설정하면?

👉 이 함수는 **아무리 호출해도 무조건 Throttled(429 오류)**
👉 즉, **항상 실패**

이걸 왜 쓰냐?

* “쓰로틀링 상황에서 우리 앱이 어떻게 반응하나?” 테스트하기 좋음.

---

# 4️⃣ Provisioned Concurrency = 미리 ‘따뜻하게 켜놓기’

Lambda의 가장 큰 문제 중 하나는 **Cold Start(콜드 스타트)**.
즉, 처음 호출될 때 느려지는 것.
...
⚠️ 적용 조건
Provisioned concurrency는 **버전 또는 alias에만 적용됨**
→ `$LATEST` 버전에는 적용 불가
→ 그래서 먼저 **버전 publish** 해야 함.

예)
함수 버전 1에 대해 "Provisioned concurrency = 5"
→ 항상 5개의 인스턴스가 따뜻하게 준비됨.
...
| 기능                      | 의미                   | 목적                 |
| ----------------------- | -------------------- | ------------------ |
| Unreserved concurrency  | 전체 계정이 공유하는 기본 1000개 | 모든 함수가 나눠 쓰는 기본용   |
| Reserved concurrency    | 특정 함수에 전용 슬롯 배정      | 중요한 함수가 막히지 않도록 보호 |
| Reserved = 0            | 함수 완전 차단             | 쓰로틀 테스트용           |
| Provisioned concurrency | 미리 인스턴스를 준비          | 콜드 스타트 제거, 성능 향상   |

---

# 🧠 핵심만 다시!

* 기본은 **1000개 동시 실행**을 계정 전체가 공유함.
* 특정 함수가 너무 많이 실행되면 다른 함수가 막힐 수 있음 → Reserved concurrency로 해결.
* Reserved concurrency = 0 → 그 함수는 무조건 에러 → 테스트용.
* Provisioned concurrency → 항상 따뜻하게 준비해둬서 지연 없음 (단 비용 든다).
