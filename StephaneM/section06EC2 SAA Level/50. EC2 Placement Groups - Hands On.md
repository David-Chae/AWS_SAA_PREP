...
**Resolution / 해결 방법**. **Try the following steps to resolve your error:**
...
2. **Go to the 'Launch Instance' page**
   '인스턴스 시작(Launch Instance)' 페이지로 이동하세요.

3. **In the 'Instance type' section, select an instance type that supports cluster placement groups. Some options include:**
   '인스턴스 타입(Instance type)' 섹션에서 클러스터 배치 그룹을 지원하는 인스턴스 타입을 선택하세요. 선택 가능한 예시는 다음과 같습니다:

4. **C5 instances (e.g., c5.large)**
   C5 인스턴스 (예: c5.large)

5. **M5 instances (e.g., m5.large)**
   M5 인스턴스 (예: m5.large)

6. **R5 instances (e.g., r5.large)**
   R5 인스턴스 (예: r5.large)

---

7. **Alternatively, if you need to use a t2.micro instance:**
    또는 `t2.micro` 인스턴스를 사용해야 하는 경우:

8. **In the 'Network settings' section, expand 'Advanced network configuration'**
    '네트워크 설정(Network settings)' 섹션에서 '고급 네트워크 구성(Advanced network configuration)'을 확장하세요.

9. **Find the 'Placement group' option**
    '배치 그룹(Placement group)' 옵션을 찾으세요.

10. **Select 'Don't include in placement group' or choose a different placement group strategy (e.g., 'Spread' or 'Partition')**
    '배치 그룹에 포함하지 않음(Don’t include in placement group)'을 선택하거나 다른 배치 그룹 전략(예: 'Spread' 또는 'Partition')을 선택하세요.

11. **Review and adjust other instance launch settings as needed**
    필요에 따라 다른 인스턴스 시작 설정도 검토하고 조정하세요.

12. **Click on 'Launch instance' to create the EC2 instance with the updated configuration**
