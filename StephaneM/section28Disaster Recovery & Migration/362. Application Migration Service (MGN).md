## **AWS Application Migration Service (MGN) 이해 쉽게 정리**

### 1️⃣ 클라우드로 이전하려는 이유

* **새로 시작**: 완전히 새로 클라우드 환경을 만들어 기존 서버는 그대로 두는 경우 → **마이그레이션 필요 없음**
* **기존 서버 이전**: 온프레미스 서버/데이터 센터를 AWS로 옮기고 싶을 때 → **계획적 마이그레이션 필요**

---

### 2️⃣ 마이그레이션 계획 세우기

**AWS Application Discovery Service** 사용 → 서버 분석 + 의존성 파악
...
1. **Agentless Discovery (커넥터 사용)**

   * VM의 기본 정보와 성능 기록 제공
   * CPU, 메모리, 디스크 사용량 등
2. **Agent-based Discovery (에이전트 설치)**

   * VM 내부에서 세부 정보 제공
   * 시스템 구성, 실행 중 프로세스, 네트워크 연결, 의존성 정보 포함
   * 의존성 매핑에 유용

#### 결과 확인

* **AWS Migration Hub**에서 수집된 데이터를 시각화
* 무엇을 먼저 옮길지, 시스템 간 연결을 어떻게 처리할지 계획 가능

---

### 3️⃣ AWS로 실제 이전

**AWS Application Migration Service (MGN)** 사용

* 이전 이름: CloudEndure Migration
* 목적: **Lift-and-Shift** (기존 서버 그대로 AWS로 옮기기)

#### 작동 방식

1. 데이터 센터 서버에 **Replication Agent 설치**
2. 서버의 **OS, 애플리케이션, 데이터**를 AWS로 **연속 복제**

   * 저비용 **EC2 인스턴스 + EBS 볼륨**에 데이터 저장
3. 준비 완료 시 **Cutover (본격 이전)**

   * 스테이징 환경 → 프로덕션 전환
   * 필요에 따라 **EC2 인스턴스 규모 및 EBS 성능 조정**
...
* 최소 다운타임으로 마이그레이션 가능
* 반복적 수동 작업 불필요 → 비용 절감
* 엔지니어링 복잡성 감소 → 쉽게 이전 가능

---

### 5️⃣ 한눈에 보는 핵심 흐름

1. **Discovery** → Application Discovery Service

   * 서버 활용량, 의존성 파악
   * Agentless vs Agent-based 선택

2. **계획 시각화** → Migration Hub

   * 데이터 시각화, 이전 우선순위 결정

3. **이전 수행** → Application Migration Service (MGN)

   * 서버 에이전트 설치 → 연속 복제
   * Cutover → 스테이징 → 프로덕션 전환

---

### 🔑 핵심 요약

* **Discovery Service**: 마이그레이션 계획 수립
* **Migration Hub**: 데이터 시각화 + 진행 상황 확인
* **Application Migration Service (MGN)**: 최소 다운타임, 자동화된 Lift-and-Shift 마이그레이션
