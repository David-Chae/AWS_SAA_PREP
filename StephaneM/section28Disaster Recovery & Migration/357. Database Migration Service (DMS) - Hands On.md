...
AWS DMS는 **데이터베이스를 안전하게 AWS로 옮기거나 계속 복제**할 수 있는 서비스입니다.

핵심은 **“데이터를 이동시키고, 필요한 경우 계속 업데이트를 반영”**하는 것이라고 생각하면 됩니다.

---

### **1️⃣ 기본 흐름**

1. **출발지(Source)와 도착지(Target) 결정**

   * 데이터가 어디서 오는지(Source)
   * 어디로 가는지(Target)
   * 각 위치를 DMS에 알려주는 **엔드포인트(Endpoint)**를 만들어야 함

2. **엔드포인트 설정**

   * **Source Endpoint**: 데이터를 가져올 데이터베이스 설정
   * **Target Endpoint**: 데이터를 넣을 데이터베이스 설정
   * 연결 테스트 가능 (선택) – 데이터가 제대로 연결되는지 확인

3. **복제 인스턴스(Replication Instance) 만들기**

   * 데이터 이동을 실제로 수행할 서버
   * 선택 옵션:

     * **Provisioned**: 직접 서버 크기와 스토리지를 선택
     * **Serverless**: AWS가 자동으로 필요한 컴퓨팅을 조정

4. **복제 작업(Task) 생성**

   * Source → Target 데이터 이동 방법 결정
   * 작업 유형:

     1. **Migrate data once**: 한 번만 데이터 이동
     2. **Migrate and replicate (CDC)**: 전체 데이터 이동 후 변경 사항 지속적으로 반영
     3. **Replicate only**: 기존 데이터 변경만 반영

5. **추가 설정**

   * 로깅(logging), 연결(connectivity) 등 세부 옵션 설정 가능

---

### **2️⃣ 선택 사항과 유연성**

* **서버리스(Serverless)**: 서버를 직접 만들 필요 없음, 자동으로 컴퓨팅 조절
* **Provisioned**: 직접 서버 크기와 고가용성 등 설정
* **Schema Conversion Tool (SCT)**: 다른 종류(DB 엔진)로 옮길 때만 필요

> 예: Oracle → MySQL 이동 시 SCT 사용, MySQL → MySQL은 필요 없음

---

### **3️⃣ 요약**

* **DMS 역할**: 데이터 이동 + 실시간 변경 복제 가능
* **핵심 단계**: 엔드포인트 생성 → 복제 인스턴스 → 작업(Task) 생성
* **유연성**: 서버리스 또는 프로비저닝, 데이터 이동 유형 선택 가능
* **추가 기능**: 로깅, 연결 테스트, 스키마 변환(SCT) 등

---

💡 **쉽게 비유하면:**
DMS는 **데이터 이동 전문 운송 서비스** 같은 느낌이에요.

* 어디서 싣고(Source) → 어디로 옮기고(Target) → 운송 수단(Replication Instance) 선택 → 한 번만 운송할지, 계속 실시간 배송할지 결정(Task)
