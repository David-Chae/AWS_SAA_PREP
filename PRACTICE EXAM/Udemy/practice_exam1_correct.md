좋아. 이건 **“정답을 외워서 맞춘 상태” → “문제 구조를 보면 자동으로 답이 보이는 상태”**로 넘어가는 작업이야.
아래는 네가 올린 **맞았지만 동전 던지기 같았던 문제들**을 전부 **시험용 판단 공식**으로 압축한 거야.
👉 이 공식들만 머리에 있으면, **다음에 똑같은 유형이 나오면 오답이 먼저 보이고 정답이 남는다.**

---

# 🔑 AWS SAA 판단 공식집 (실전용)

아래는 **“문제에서 이 단어가 보이면 → 이걸 고른다 / 이건 버린다”** 구조로 정리했어.

---

## 1️⃣ CloudFront + 로그인 지연 + 글로벌 사용자
...
> **“글로벌 사용자 + 인증/요청 로직 = Lambda@Edge”**

> **“CloudFront 5xx / 504 = Origin Failover”**

### ❌ 즉시 제거

* Multi-region VPC + Transit VPC (비용·복잡도 과함)
* Route53 Latency routing (CloudFront 이미 있음)
* Cache-Control max-age (정적 캐시 문제일 때만)

👉 **CloudFront가 있으면 ‘Edge에서 처리’가 항상 우선**

---

## 2️⃣ Oracle → AWS + 고가용성 + 급함
...
> **“기존 상용 DB + HA = RDS Multi-AZ”**

### ❌ 즉시 제거

* Aurora 변환 (시간 오래 걸림)
* RAC / RMAN (RDS에서 지원 안 함)

👉 **시험에서 “긴급 + 안정성”이면 변환하지 말고 그대로 올린다**

---

## 3️⃣ 특정 IP 대역 차단 (임시, 빠르게)
...
* Security Group (deny 불가)
* IAM Policy (네트워크 제어 아님)
* OS Firewall (범위 너무 좁음)

👉 **빠르고 넓게 막아야 하면 NACL**

---

## 4️⃣ 온프레미스 AD 계정으로 AWS 접근
...
> **“AD + AWS = SAML Federation (AD FS)”**

### ❌ 즉시 제거

* IAM User (계정 중복 생성)
* Web Identity (Google/Facebook용)
* VPC (인증과 무관)
...
> **“대규모 DDoS = AWS Shield (Advanced)”**

### ❌ 즉시 제거

* WAF 단독 (L7만)
* SG + NACL (규모 대응 불가)
* Firewall Manager (관리 도구일 뿐)

👉 **DDoS = Shield, 필기시험 수준에서 거의 공식**

---

## 6️⃣ IAM Policy 해석 문제
...
> **Action + Resource를 “교차”해서 본다**

* `Get*` + `*` → **모든 버킷 읽기**
* `PutObject` + 특정 ARN → **그 버킷에만 쓰기**
* `Delete` 없음 → **삭제 불가**

👉 **IAM은 “있다/없다”만 본다. 추측 금지**
...
> **“S3 삭제 방지 = Versioning + MFA Delete”**
...
> **“Lambda secret = KMS CMK (customer-managed)”**
...
> **“NAT = AZ당 1개 + Private Subnet Route”**
...
> **“최대 통제 = KMS Custom Key Store + CloudHSM”**
...
* 🔥 **이 판단 공식들 1페이지 PDF로 압축**
* 🔥 **시험 직전 15분 복기용 체크리스트**
* 🔥 **“이 서비스는 왜 절대 정답이 아닌가” 미끼 제거집**

원하면 **“이 중에서 특히 헷갈렸던 문제 3개”**만 말해줘.
그건 **완전히 자동 반사 수준**으로 만들어줄게.
